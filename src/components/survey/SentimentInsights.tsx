...
const SentimentInsights: React.FC<SentimentInsightsProps> = ({
  ...
  const trends = useMemo(() => {
    ...
    return periods.map(period => {
      ...
      const positive = periodAnalyses.filter(a => a.sentiment === 'Positivo').length;
      const negative = periodAnalyses.filter(a => a.sentiment === 'Negativo').length;
      const neutral = periodAnalyses.filter(a => a.sentiment === 'Neutro').length;
  ...
  const insights = useMemo(() => {
    ...
    if (analyses.length === 0) {
      insights.push({
        ...
        description: 'Execute análises de sentimento para ver insights automáticos.',
        icon: MessageSquare,
        priority: 'low'
      });
      return insights;
    ...
    if (avgConfidence > 0.8) {
      ...
      insights.push({
        ...
        description: 'Considere revisar as análises manualmente ou melhorar os prompts de IA.',
        icon: AlertTriangle,
        priority: 'high',
        actionable: true
      });
    ...
    const topPerformer = attendantSentiments.find(a => a.positive / a.total > 0.8);
    if (topPerformer) {
      insights.push({
        type: 'success',
    ...
    const needsAttention = attendantSentiments.find(a => 
      a.negative / a.total > 0.4 || a.conflictingAnalyses > 2
    );
    if (needsAttention) {
      insights.push({
        type: 'warning',
    ...
    const totalConflicting = attendantSentiments.reduce((sum, a) => sum + a.conflictingAnalyses, 0);
                ...
                <div className="grid grid-cols-3 gap-4">
                  <div className="space-y-2">
                    <div className="flex justify-between text-sm">
                      ...
                      <span>{trend.positive} ({trend.positivePercentage.toFixed(1)}%)</span>
                    </div>
                    <Progress value={trend.positivePercentage} className="h-2" />
                  </div>
                  
                  <div className="space-y-2">
                    <div className="flex justify-between text-sm">
                      <span className="text-gray-600">Neutro</span>
                      <span>{trend.neutral} ({((trend.neutral / trend.total) * 100).toFixed(1)}%)</span>
                    </div>
                    <Progress value={(trend.neutral / trend.total) * 100} className="h-2" />
                  </div>
                  
                  <div className="space-y-2">
                    <div className="flex justify-between text-sm">
                      <span className="text-red-600">Negativo</span>
                      <span>{trend.negative} ({trend.negativePercentage.toFixed(1)}%)</span>
                    </div>
                    <Progress value={trend.negativePercentage} className="h-2" />
                  </div>
                </div>
          ...
          {attendantSentiments.length === 0 ? (
            ...
            <div className="space-y-4">
              {attendantSentiments.slice(0, 10).map((attendant) => {
                const positivePercentage = (attendant.positive / attendant.total) * 100;
                const negativePercentage = (attendant.negative / attendant.total) * 100;
                
                return (
                  <div key={attendant.attendantId} className="space-y-3 p-4 border rounded-lg">
                    <div className="flex items-center justify-between">
                      <div>
                        ...
                        <div className="flex items-center gap-2 text-sm text-muted-foreground">
                          ...
                          <span>•</span>
                          <span>Nota média: {attendant.avgRating.toFixed(1)}</span>
                          <span>•</span>
                          <span>Confiança: {(attendant.avgConfidence * 100).toFixed(1)}%</span>
                        </div>
                      </div>
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <SentimentBadge 
                          sentiment={positivePercentage > 50 ? 'Positivo' : 
                                   negativePercentage > 50 ? 'Negativo' : 'Neutro'}
                          size="sm"
                        />
                      </div>
                    </div>
                    ...
                    <div className="grid grid-cols-3 gap-2">
                      <div className="text-center">
                        <div className="text-lg font-bold text-green-600">{attendant.positive}</div>
                        <div className="text-xs text-muted-foreground">Positivo</div>
                      </div>
                      <div className="text-center">
                        <div className="text-lg font-bold text-gray-600">{attendant.neutral}</div>
                        <div className="text-xs text-muted-foreground">Neutro</div>
                      </div>
                      <div className="text-center">
                        <div className="text-lg font-bold text-red-600">{attendant.negative}</div>
