...
describe('SurveyForm', () => {
  ...
  describe('Renderização', () => {
    it('deve renderizar todos os campos obrigatórios', () => {
      ...
      expect(screen.getByPlaceholderText('Conte-nos sobre sua experiência...')).toBeInTheDocument();
    ...
    it('deve renderizar descrição personalizada quando fornecida', () => {
      render(<SurveyForm {...defaultProps} description="Descrição personalizada" />);
    ...
    it('deve mostrar campos opcionais quando showOptionalFields é true', () => {
      ...
      expect(screen.getByPlaceholderText('Seu email (opcional)')).toBeInTheDocument();
    });

    it('deve mostrar campo de sugestões quando showSuggestions é true', () => {
      render(<SurveyForm {...defaultProps} showSuggestions />);
      
      expect(screen.getByPlaceholderText('Sugestões para melhorar nosso atendimento...')).toBeInTheDocument();
    });
  });
  ...
  describe('Interações do usuário', () => {
    ...
    it('deve atualizar comentário quando texto é digitado', async () => {
      ...
      const commentTextarea = screen.getByPlaceholderText('Conte-nos sobre sua experiência...');
  ...
  describe('Validação', () => {
    it('deve desabilitar botão de envio quando rating não está definido', () => {
      ...
      const submitButton = screen.getByRole('button', { name: /enviar avaliação/i });
  ...
  describe('Submissão do formulário', () => {
    ...
    it('deve incluir campos opcionais quando preenchidos', async () => {
      ...
      await user.type(screen.getByPlaceholderText('Conte-nos sobre sua experiência...'), 'Comentário');
      await user.type(screen.getByPlaceholderText('Seu nome (opcional)'), 'João');
      await user.type(screen.getByPlaceholderText('Seu email (opcional)'), 'joao@email.com');
      await user.type(screen.getByPlaceholderText('Sugestões para melhorar nosso atendimento...'), 'Sugestão');
  ...
  describe('Estados especiais', () => {
    it('deve renderizar em modo readonly quando especificado', () => {
      ...
      const submitButton = screen.queryByRole('button', { name: /enviar avaliação/i });
    ...
    it('deve carregar dados iniciais quando fornecidos', () => {
      ...
      expect(screen.getByDisplayValue('Comentário inicial')).toBeInTheDocument();
    ...
    it('deve mostrar mensagem de sucesso após submissão', async () => {
      ...
      render(<SurveyForm {...defaultProps} onSubmit={mockOnSubmit} showSuccessMessage />);
    ...
    it('deve mostrar mensagem de erro quando submissão falha', async () => {
      const user = userEvent.setup();
      const mockOnSubmit = jest.fn().mockRejectedValue(new Error('Erro de rede'));
      ...
      await waitFor(() => {
        expect(screen.getByText(/erro ao enviar avaliação/i)).toBeInTheDocument();
      });
    });
  });
