...
describe("CreateBackupForm", () => {
  ...
  it("deve renderizar o formulário corretamente", () => {
    ...
    expect(
      screen.getByText("Configure as opções do backup do banco de dados"),
    ).toBeInTheDocument();
    expect(
      screen.getByLabelText("Nome do Arquivo (Opcional)"),
    ).toBeInTheDocument();
    expect(
      screen.getByLabelText("Incluir Estrutura (Schema)"),
    ).toBeInTheDocument();
  ...
  it("deve ter valores padrão corretos", () => {
    ...
    const schemaCheckbox = screen.getByLabelText(
      "Incluir Estrutura (Schema)",
    ) as HTMLInputElement;
    const dataCheckbox = screen.getByLabelText(
      "Incluir Dados",
    ) as HTMLInputElement;
    const compressCheckbox = screen.getByLabelText(
      "Comprimir Arquivo",
    ) as HTMLInputElement;
  ...
  it("deve mostrar configurações avançadas quando clicado", () => {
    ...
    expect(
      screen.getByText("• Diretório de destino: /app/backups"),
    ).toBeInTheDocument();
  });
