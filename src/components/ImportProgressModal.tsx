...
import { validateImportStatus, DEFAULT_IMPORT_STATUS } from "@/lib/data-validation";
import React from "react";

export default function ImportProgressModal() {
    const { importStatus, setImportStatus } = usePrisma();
    ...
    React.useEffect(() => {
        if (!validationResult.isValid && validationResult.errors.length > 0) {
            console.warn('ImportProgressModal: ImportStatus inválido detectado:', validationResult.errors);
        }
        ...
        <Dialog open={isOpen} onOpenChange={(isOpen) => {
            ...
            <DialogContent className="sm:max-w-md" onInteractOutside={(e) => {
                ...
                <DialogHeader>
                    ...
                    <DialogDescription>
                        {status === 'processing' && "Por favor, aguarde a conclusão do processo. Não feche esta aba."}
