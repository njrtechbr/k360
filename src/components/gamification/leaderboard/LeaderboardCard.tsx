...
export default function LeaderboardCard({
  ...
  const currentUserEntry = currentAttendantId
    ? entries.find((entry) => entry.attendantId === currentAttendantId)
    : null;
  ...
  return (
    <Card className={cn("", className)}>
      <CardHeader className="pb-3">
        <div className="flex items-center justify-between">
          <div>
            ...
            <CardDescription>{entries.length} participantes</CardDescription>
          </div>
      ...
      <CardContent className="space-y-3">
        ...
        {currentUserEntry &&
          !topEntries.some(
            (entry) => entry.attendantId === currentAttendantId,
          ) && (
            <>
              <div className="border-t pt-3">
                ...
                <div className="flex items-center gap-3 p-3 rounded-lg bg-primary/5 border border-primary/20">
                  <div className="flex items-center gap-2 min-w-0 flex-1">
                    ...
                    <Avatar className="h-8 w-8">
                      <AvatarFallback className="text-xs">
                        {currentUserEntry.attendantName
                          .split(" ")
                          .map((n) => n[0])
                          .join("")
                          .slice(0, 2)}
