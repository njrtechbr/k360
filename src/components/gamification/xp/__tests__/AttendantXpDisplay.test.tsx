...
describe('AttendantXpDisplay', () => {
  ...
  beforeEach(() => {
    ...
    (global.fetch as jest.Mock).mockImplementation((url) => {
      if (typeof url === 'string') {
        ...
        if (url.includes('/xp-events/attendant/') && url.includes('type=evaluation')) {
          return Promise.resolve({
            ok: true,
  ...
  it('deve exibir o histórico de XP avulso quando showHistory é true', async () => {
    render(<AttendantXpDisplay attendantId={mockAttendantId} showHistory={true} />);
  ...
  it('deve renderizar na variante compact', async () => {
    render(<AttendantXpDisplay attendantId={mockAttendantId} variant="compact" />);
    ...
    expect(screen.queryByText('Histórico de XP Avulso')).not.toBeInTheDocument();
  });
  ...
  it('deve exibir erro quando falha ao carregar dados', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText('Erro ao carregar dados de XP')).toBeInTheDocument();
    });
  });
