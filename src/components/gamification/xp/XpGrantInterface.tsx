...
export function XpGrantInterface({
  ...
  const [selectedXpType, setSelectedXpType] = useState<XpTypeConfig | null>(
    null,
  );
  ...
  const fetchData = async () => {
    try {
      ...
      if (attendantsResponse.ok) {
        ...
        const activeAttendants = attendantsData.filter(
          (a: Attendant) => a.status === "Ativo",
        );
      ...
      if (xpTypesResponse.ok) {
        ...
        const activeXpTypes = Array.isArray(xpTypes)
          ? xpTypes.filter((type: XpTypeConfig) => type.active)
          : [];
  ...
  const filteredAttendants = attendants.filter(
    (attendant) =>
      attendant.name
        .toLowerCase()
        .includes(attendantSearchValue.toLowerCase()) ||
      attendant.email
        .toLowerCase()
        .includes(attendantSearchValue.toLowerCase()) ||
      attendant.funcao
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem className="flex flex-col">
                    ...
                    <Popover
                      open={attendantSearchOpen}
                      onOpenChange={setAttendantSearchOpen}
                    >
                              ...
                              <div className="flex items-center gap-2">
                                ...
                                <div className="text-left">
                                  ...
                                  <p className="text-xs text-muted-foreground">
                                    {selectedAttendant.funcao} -{" "}
                                    {selectedAttendant.setor}
                                  </p>
                                </div>
                              </div>
                            ) : (
                          ...
                          <CommandGroup className="max-h-64 overflow-auto">
                            {filteredAttendants.map((attendant) => (
                              <CommandItem
                                ...
                                onSelect={() =>
                                  handleAttendantSelect(attendant)
                                }
                              >
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <Select
                      onValueChange={handleXpTypeSelect}
                      value={field.value}
                    >
                      <FormControl>
                        <SelectTrigger>
              ...
              {selectedXpType && (
                <div
                  className={`p-4 border rounded-lg ${
                    ...
                    selectedXpType.points > remainingPoints
                      ...
                      {remainingPoints !== undefined &&
                        selectedXpType.points > remainingPoints && (
                          <p className="text-sm text-red-600 mt-1">
                            Este tipo requer {selectedXpType.points} pontos, mas
                            você tem apenas {remainingPoints} restantes hoje.
                          </p>
                        )}
                    </div>
                  </div>
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <p className="text-xs text-muted-foreground">
                      Adicione uma justificativa para facilitar a auditoria e o
                      acompanhamento
                    </p>
                    <FormMessage />
                  </FormItem>
          ...
          <AlertDialogHeader>
            ...
            <AlertDialogDescription asChild>
              <div className="space-y-4">
                <p>
                  Você está prestes a conceder XP avulso. Confirme os detalhes:
                </p>
