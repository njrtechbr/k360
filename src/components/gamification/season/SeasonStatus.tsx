...
export default function SeasonStatus({
  ...
  if (!activeSeason) {
    if (nextSeason) {
      ...
      const formattedStartDate = format(startDate, "dd 'de' MMMM 'de' yyyy", {
        locale: ptBR,
      });
      ...
      <Card className={cn("bg-muted/50 border-dashed", className)}>
        <CardHeader className="flex-row items-center gap-4">
          ...
          <div>
            ...
            <CardDescription>
              O ranking e o acúmulo de XP estão pausados. Um administrador pode
              iniciar uma nova temporada nas configurações.
            </CardDescription>
          </div>
        </CardHeader>
      </Card>
  ...
  const endDate =
    activeSeason.endDate instanceof Date
      ? activeSeason.endDate
      : parseISO(activeSeason.endDate);
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            <CardTitle className="text-xl">
              Temporada Atual: {activeSeason.name}
            </CardTitle>
            <CardDescription>
