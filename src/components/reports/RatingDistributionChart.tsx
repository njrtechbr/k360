...
export function RatingDistributionChart({ distribution, loading = false }: RatingDistributionChartProps) {
    if (loading) {
        return (
            <Card>
                <CardHeader>
    ...
    const data = Object.entries(distribution).map(([rating, count]) => ({
        ...
        count,
        percentage: distribution ? ((count / Object.values(distribution).reduce((a, b) => a + b, 0)) * 100).toFixed(1) : 0
    }));
    ...
    const totalEvaluations = Object.values(distribution).reduce((a, b) => a + b, 0);
                    ...
                    <div className="space-y-2">
                        <div className="flex justify-between">
                            ...
                            <span className="font-medium">
                                {Object.entries(distribution).reduce((a, b) => distribution[a[0] as any] > distribution[b[0] as any] ? a : b)[0]} estrelas
                            </span>
                        </div>
                        <div className="flex justify-between">
                            <span className="text-muted-foreground">Satisfação (4-5):</span>
                            <span className="font-medium text-green-600">
                                {(((distribution[4] || 0) + (distribution[5] || 0)) / totalEvaluations * 100).toFixed(1)}%
                            </span>
                        </div>
                    </div>
                    <div className="space-y-2">
                        <div className="flex justify-between">
                            ...
                            <span className="font-medium text-red-600">
                                {(((distribution[1] || 0) + (distribution[2] || 0)) / totalEvaluations * 100).toFixed(1)}%
                            </span>
                        </div>
                        <div className="flex justify-between">
