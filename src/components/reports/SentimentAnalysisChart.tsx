...
export function SentimentAnalysisChart({
  ...
  const topAttendants = sentimentData
    ...
    .map((attendant) => ({
      name:
        attendant.attendantName.length > 15
      ...
      positiveRate: (
        (attendant.positive / attendant.totalAnalyzed) *
        100
      ).toFixed(1),
    }));
  ...
  const PieTooltip = ({ active, payload }: any) => {
    if (active && payload && payload.length) {
      ...
      const percentage = ((data.value / overallSentiment.total) * 100).toFixed(
        1,
      );
      return (
        <div className="bg-background border rounded-lg p-3 shadow-lg">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>Estatísticas dos sentimentos</CardDescription>
          </CardHeader>
            ...
            <div className="flex items-center justify-between">
              ...
              <div className="text-right">
                ...
                <div className="text-xs text-muted-foreground">
                  {overallSentiment.total > 0
                    ? (
                        (overallSentiment.positive / overallSentiment.total) *
                        100
                      ).toFixed(1)
                    : 0}
                  %
            ...
            <div className="flex items-center justify-between">
              ...
              <div className="text-right">
                ...
                <div className="text-xs text-muted-foreground">
                  {overallSentiment.total > 0
                    ? (
                        (overallSentiment.neutral / overallSentiment.total) *
                        100
                      ).toFixed(1)
                    : 0}
                  %
            ...
            <div className="flex items-center justify-between">
              ...
              <div className="text-right">
                ...
                <div className="text-xs text-muted-foreground">
                  {overallSentiment.total > 0
                    ? (
                        (overallSentiment.negative / overallSentiment.total) *
                        100
                      ).toFixed(1)
                    : 0}
                  %
      ...
      {topAttendants.length > 0 && (
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Análise de sentimentos dos comentários por atendente (top 10 mais
              analisados)
            </CardDescription>
          </CardHeader>
          <CardContent>
