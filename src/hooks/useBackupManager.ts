...
export function useBackupManager() {
    ...
    async (options?: BackupOptions) => {
      try {
        ...
        toast({
          ...
          description:
            "O processo de backup foi iniciado. Acompanhe o progresso abaixo.",
        });
  ...
  const cancelBackup = useCallback(async () => {
    ...
    try {
      ...
      const response = await fetch(
        `/api/backup/cancel/${backupProgress.backupId}`,
        {
          method: "POST",
        },
      );
  ...
  const updateProgress = useCallback(
    (progress: number, message: string, status: string) => {
      setBackupProgress((prev) => ({
        ...prev,
        progress,
        message,
      ...
      if (status === "completed" || status === "failed") {
        ...
        if (status === "completed") {
          ...
          toast({
            ...
            description:
              "O backup foi criado com sucesso e está disponível para download.",
          });
        }
