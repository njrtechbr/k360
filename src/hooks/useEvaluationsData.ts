...
export function useEvaluationsData({
  ...
  const fetchEvaluations = useCallback(async () => {
    ...
    try {
      ...
      console.log(
        `PERF: fetchEvaluations (${list.length} items) took ${(endTime - startTime).toFixed(2)}ms`,
      );
      return list;
    } catch (error) {
  ...
  const fetchEvaluationImports = useCallback(async () => {
    ...
    try {
      ...
      const endTime = performance.now();
      console.log(
        `PERF: fetchEvaluationImports (${list.length} items) took ${(endTime - startTime).toFixed(2)}ms`,
      );
      return list;
    } catch (error) {
  ...
  const addEvaluation = useCallback(
    async (
      evaluationData: Omit<Evaluation, "id" | "xpGained">,
    ): Promise<Evaluation> => {
      try {
  ...
  const deleteEvaluations = useCallback(
    async (evaluationIds: string[]) => {
      try {
        ...
        const aiAnalysisToKeep = currentAiAnalysis.filter(
          (ar) => !evaluationIds.includes(ar.evaluationId),
        );
  ...
  const addImportRecord = useCallback(
    async (
      importData: Omit<EvaluationImport, "id">,
      ...
      try {
        ...
        console.error(
          "EVALUATIONS: Erro ao salvar histórico de importação:",
          error,
        );
        throw error;
      }
    ...
    async (importId: string) => {
      try {
        const response = await fetch(
          `/api/evaluations/imports/${importId}/revert`,
          {
            method: "DELETE",
          },
        );
        ...
        const currentAiAnalysis = getAiAnalysisFromStorage();
        const aiAnalysisToKeep = currentAiAnalysis.filter(
          (ar) => !result.deletedEvaluationIds.includes(ar.evaluationId),
        );
        saveAiAnalysisToStorage(aiAnalysisToKeep);
        ...
        toast({
          ...
          description:
            "As avaliações da importação selecionada foram removidas.",
        });
        ...
        toast({
          variant: "destructive",
          title: "Erro",
          description: "Erro ao reverter importação.",
        });
        throw error;
      }
    },
  ...
  const runAiAnalysis = async () => {
    ...
    const pendingEvaluations = allEvaluations.filter(
      (e) =>
        ...
        e.comentario &&
        e.comentario.trim() !== "(Sem comentário)" &&
        e.comentario.trim() !== "",
    );

    if (pendingEvaluations.length === 0) {
      toast({
        title: "Nenhuma nova avaliação",
        description: "Todos os comentários já foram analisados.",
      });
    ...
    try {
      ...
      toast({
        variant: "destructive",
        title: "Erro na Análise de IA",
        description: "Ocorreu um erro geral. Tente novamente.",
      });
    } finally {
      setIsAiAnalysisRunning(false);
      setIsProgressModalOpen(false);
