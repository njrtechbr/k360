...
const mergeAchievementsWithDefaults = (
  savedAchievements: Partial<Achievement>[],
): Achievement[] => {
...
const mergeLevelRewardsWithDefaults = (
  savedRewards: Partial<LevelReward>[],
): LevelReward[] => {
...
export function useGamificationData() {
  ...
  const fetchGamificationConfig = useCallback(async () => {
    ...
    try {
      ...
      console.log("GAMIFICATION: Dados carregados via API", {
        achievements: data.achievements?.length,
        levelRewards: data.levelRewards?.length,
      });
      ...
      console.log(
        `PERF: fetchGamificationConfig took ${(endTime - startTime).toFixed(2)}ms`,
      );
      return loadedConfig;
    } catch (error) {
  ...
  const fetchUnlockedAchievements = useCallback(async () => {
    console.log(
      "GAMIFICATION: fetchUnlockedAchievements não implementado para frontend",
    );
    // Esta função não deve ser usada no frontend - usar API específica se necessário
    return [];
  }, []);

  const calculateActiveAndNextSeason = useCallback(
    (allSeasons: GamificationSeason[]) => {
      const now = new Date();
      ...
      console.log(
        "GAMIFICATION: Temporada ativa e próxima temporada calculadas.",
        { active: currentActiveSeason?.name, next: nextUpcomingSeason?.name },
      );
    },
    [],
  );
  ...
  const updateGamificationConfig = async (
    newConfig: Partial<
      Pick<GamificationConfig, "ratingScores" | "globalXpMultiplier">
    >,
  ) => {
    try {
      const response = await fetch("/api/gamification", {
  ...
  const saveSeasons = async (newSeasons: GamificationSeason[]) => {
    try {
      ...
      toast({
        variant: "destructive",
        title: "Erro",
        description: "Não foi possível salvar as temporadas.",
      });
    }
  };

  const addSeason = (seasonData: Omit<GamificationSeason, "id">) => {
    const newSeason: GamificationSeason = {
      ...seasonData,
    ...
    toast({
      ...
      description: "A nova sessão de gamificação foi criada.",
    });
  };

  const updateSeason = (
    id: string,
    seasonData: Partial<Omit<GamificationSeason, "id">>,
  ) => {
    const updatedSeasons = seasons.map((s) =>
      s.id === id ? { ...s, ...seasonData } : s,
    );
  ...
  const deleteSeason = (id: string) => {
    ...
    toast({
      ...
      description: "A sessão de gamificação foi removida.",
    });
  };
  ...
  const updateAchievement = async (
    ...
    try {
      ...
      console.log(
        "GAMIFICATION: Achievement atualizado com sucesso via API",
        result,
      );

      await fetchGamificationConfig();
      toast({
        title: "Troféu Atualizado!",
        description: "As informações do troféu foram salvas.",
      });
    } catch (error) {
      console.error(
        "GAMIFICATION: Erro ao atualizar achievement via API:",
        error,
      );
      toast({
        variant: "destructive",
        title: "Erro",
        description: "Não foi possível salvar o troféu.",
      });
    }
  };

  const updateLevelReward = async (
    level: number,
    data: Partial<Omit<LevelReward, "level" | "icon" | "color">>,
  ) => {
    try {
      ...
      toast({
        ...
        description: "A recompensa do nível foi salva.",
      });
    } catch (error) {
  ...
  const recalculateAllGamificationData = useCallback(
    async (
      ...
      console.log(
        "GAMIFICATION: recalculateAllGamificationData não implementado para frontend",
      );
      // Esta função não deve ser usada no frontend - usar API específica se necessário
