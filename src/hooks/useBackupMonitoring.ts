...
export function useBackupMonitoring(autoRefresh: boolean = true): UseBackupMonitoringReturn {
  ...
  const fetchHealthCheck = useCallback(async (): Promise<HealthCheckResult | null> => {
    try {
      const response = await fetch('/api/backup/monitoring?action=health');
      if (!response.ok) {
        throw new Error(`Erro ao buscar health check: ${response.statusText}`);
      }
      const data = await response.json();
  ...
  const startMonitoring = useCallback(async (): Promise<boolean> => {
    try {
      ...
      if (!response.ok) {
        throw new Error(`Erro ao iniciar monitoramento: ${response.statusText}`);
      }
  ...
  const forceHealthCheck = useCallback(async (): Promise<HealthCheckResult | null> => {
    try {
      const response = await fetch('/api/backup/monitoring', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
      ...
      if (!response.ok) {
        throw new Error(`Erro ao executar health check: ${response.statusText}`);
      }
