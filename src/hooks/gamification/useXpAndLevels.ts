...
export function useXpAndLevels(): UseXpAndLevelsReturn {
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      try {
        await Promise.all([fetchXpEvents(), fetchLevelRewards()]);
      } catch (err) {
        const errorMessage =
  ...
  const getAttendantXp = useCallback(
    async (attendantId: string, seasonId?: string) => {
      try {
        ...
        const currentLevel = LevelsService.getLevelFromXp(
          totalXp,
          levelRewards,
        );
        const levelProgress = LevelsService.getLevelProgress(
          totalXp,
          levelRewards,
        );
  ...
  const updateLevelReward = useCallback(
    async (id: string, data: Partial<LevelReward>) => {
      try {
        setIsLoading(true);
  ...
  const getXpRequiredForLevel = useCallback(
    (level: number) => {
  ...
  const getRecentlyUnlockedRewards = useCallback(
    async (attendantId: string, days = 7) => {
      try {
        const response = await fetch(
          `/api/gamification/level-rewards/recent/${attendantId}?days=${days}`,
        );
  ...
  const getUpcomingRewards = useCallback(
    (currentLevel: number, count = 5) => {
      return LevelsService.getUpcomingRewards(
        currentLevel,
        levelRewards,
        count,
      );
