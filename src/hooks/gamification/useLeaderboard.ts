...
export function useLeaderboard(): UseLeaderboardReturn {
  ...
  const [departmentLeaderboards, setDepartmentLeaderboards] = useState<
  ...
  const fetchMainLeaderboard = useCallback(
    async (seasonId?: string, limit = 50) => {
      try {
  ...
  const fetchLeaderboardStats = useCallback(async (seasonId?: string) => {
    try {
      ...
      const response = await fetch(
        `/api/gamification/leaderboard/stats?${params}`,
      );
      if (!response.ok) {
        throw new Error(`Erro na API: ${response.status}`);
      }
  ...
  const getMainLeaderboard = useCallback(
    async (seasonId?: string, limit = 50) => {
  ...
  const getDepartmentLeaderboard = useCallback(
    async (department: string, seasonId?: string, limit = 20) => {
      try {
        const params = new URLSearchParams();
        ...
        const response = await fetch(
          `/api/gamification/leaderboard/department?${params}`,
        );
        if (!response.ok) {
          throw new Error(`Erro na API: ${response.status}`);
        }
  ...
  const getAttendantPosition = useCallback(
    async (attendantId: string, seasonId?: string) => {
      try {
        const params = new URLSearchParams();
        if (seasonId) params.append("seasonId", seasonId);

        const response = await fetch(
          `/api/gamification/leaderboard/position/${attendantId}?${params}`,
        );
        if (!response.ok) {
          throw new Error(`Erro na API: ${response.status}`);
        }
  ...
  const getMostImprovedAttendants = useCallback(
    async (days = 30, limit = 10) => {
      try {
        ...
        const response = await fetch(
          `/api/gamification/leaderboard/most-improved?${params}`,
        );
        if (!response.ok) {
          throw new Error(`Erro na API: ${response.status}`);
        }
  ...
  const getPeriodRanking = useCallback(
    async (startDate: Date, endDate: Date, limit = 50) => {
      try {
        const params = new URLSearchParams();
        ...
        const response = await fetch(
          `/api/gamification/leaderboard/period?${params}`,
        );
        if (!response.ok) {
          throw new Error(`Erro na API: ${response.status}`);
        }
  ...
  const comparePerformance = useCallback(
    async (attendantId: string, compareWith: "average" | "top10" | string) => {
      try {
        const params = new URLSearchParams();
        params.append("compareWith", compareWith);

        const response = await fetch(
          `/api/gamification/leaderboard/compare/${attendantId}?${params}`,
        );
        if (!response.ok) {
          throw new Error(`Erro na API: ${response.status}`);
        }
  ...
  const getLeaderboardInsights = useCallback(async (seasonId?: string) => {
    try {
      ...
      const response = await fetch(
        `/api/gamification/leaderboard/insights?${params}`,
      );
      if (!response.ok) {
        throw new Error(`Erro na API: ${response.status}`);
      }
