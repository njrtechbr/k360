...
export function useAchievements(): UseAchievementsReturn {
  ...
  const getAchievementStats = useCallback(() => {
    ...
    const byDifficulty = achievements.reduce(
      (acc, achievement) => {
  ...
  const filterAchievements = useCallback(
    (filters: { category?: string; difficulty?: string; active?: boolean }) => {
      let filtered = [...achievements];

      if (filters.category) {
        filtered = AchievementsService.filterByCategory(
          filtered,
          filters.category,
        );
      }
  ...
  const refreshAchievements = useCallback(async () => {
    await Promise.all([fetchAchievements(), fetchUnlockedAchievements()]);
  }, [fetchAchievements, fetchUnlockedAchievements]);
