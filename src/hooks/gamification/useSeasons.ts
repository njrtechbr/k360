...
export function useSeasons(): UseSeasonsReturn {
  ...
  const createSeason = useCallback(async (seasonData: Omit<GamificationSeason, 'id'>) => {
    try {
      setIsLoading(true);
      ...
      toast({
        variant: 'destructive',
        title: 'Erro',
        description: err instanceof Error ? err.message : 'Não foi possível criar a temporada.'
      });
    } finally {
      setIsLoading(false);
    }
  ...
  // Ativar temporada
  const activateSeason = useCallback(async (id: string) => {
    try {
      setIsLoading(true);
      
      const response = await fetch(`/api/gamification/seasons/${id}/activate`, {
        method: 'POST'
      });
  ...
  const getSeasonProgress = useCallback((season?: GamificationSeason) => {
  ...
  const getSeasonStats = useCallback(() => {
    ...
    const active = seasons.filter(s => SeasonsService.isSeasonActive(s)).length;
