...
describe('useSentimentFilters', () => {
  ...
  describe('Filtro por sentimento', () => {
    it('deve filtrar por sentimento positivo', () => {
      ...
      expect(result.current.filteredAnalyses.every(a => a.sentiment === 'positive')).toBe(true);
    });
  ...
  describe('Filtro por confiança', () => {
    it('deve filtrar por faixa de confiança', () => {
      ...
      expect(result.current.filteredAnalyses).toHaveLength(2);
      expect(result.current.filteredAnalyses.every(a => a.confidence >= 80)).toBe(true);
    });
    ...
    it('deve filtrar por confiança mínima', () => {
      ...
      expect(result.current.filteredAnalyses).toHaveLength(2);
      expect(result.current.filteredAnalyses.map(a => a.id)).toEqual(['analysis-1', 'analysis-2']);
    });
  });
  ...
  describe('Filtro por rating', () => {
    it('deve filtrar por faixa de rating', () => {
      ...
      expect(result.current.filteredAnalyses).toHaveLength(2);
      expect(result.current.filteredAnalyses.every(a => a.evaluation.rating >= 4)).toBe(true);
    });
  ...
  describe('Filtro por data', () => {
    ...
    it('deve filtrar por data final', () => {
      ...
      expect(result.current.filteredAnalyses).toHaveLength(2);
      expect(result.current.filteredAnalyses.map(a => a.id)).toEqual(['analysis-3', 'analysis-4']);
    });
    ...
    it('deve filtrar por faixa de datas', () => {
      ...
      expect(result.current.filteredAnalyses).toHaveLength(2);
      expect(result.current.filteredAnalyses.map(a => a.id)).toEqual(['analysis-2', 'analysis-3']);
    });
  });
  ...
  describe('Filtro por atendentes', () => {
    it('deve filtrar por atendente específico', () => {
      ...
      expect(result.current.filteredAnalyses).toHaveLength(2);
      expect(result.current.filteredAnalyses.every(a => a.evaluation.attendantId === 'att-1')).toBe(true);
    });
    ...
    it('deve filtrar por múltiplos atendentes', () => {
      ...
      expect(result.current.filteredAnalyses.every(a => 
  ...
  describe('Ordenação', () => {
    it('deve ordenar por data decrescente (padrão)', () => {
      ...
      const dates = result.current.filteredAnalyses.map(a => new Date(a.createdAt));
    ...
    it('deve ordenar por confiança', () => {
      ...
      act(() => {
        result.current.updateFilters({ sortBy: 'confidence', sortOrder: 'desc' });
      });
      
      const confidences = result.current.filteredAnalyses.map(a => a.confidence);
    ...
    it('deve ordenar por rating', () => {
      ...
      const ratings = result.current.filteredAnalyses.map(a => a.evaluation.rating);
    ...
    it('deve ordenar por sentimento', () => {
      ...
      const sentiments = result.current.filteredAnalyses.map(a => a.sentiment);
      // Verificar se está ordenado alfabeticamente
      for (let i = 1; i < sentiments.length; i++) {
        expect(sentiments[i-1].localeCompare(sentiments[i])).toBeLessThanOrEqual(0);
      }
    });
  });
  ...
  describe('Performance', () => {
    it('deve memoizar resultados filtrados', () => {
      const { result, rerender } = renderHook(() => useSentimentFilters(mockAnalyses));
  ...
  describe('Casos extremos', () => {
    ...
    it('deve lidar com dados incompletos', () => {
      ...
      const { result } = renderHook(() => useSentimentFilters(incompleteAnalyses));
