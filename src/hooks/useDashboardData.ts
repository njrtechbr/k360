...
export function useDashboardData() {
  ...
  const loadData = useCallback(async (endpoint: string, dataKey: keyof DashboardDataState) => {
    ...
    try {
      ...
      setData(prev => ({
        ...prev,
        loading: { ...prev.loading, [dataKey]: false },
        errors: { ...prev.errors, [dataKey]: error instanceof Error ? error.message : 'Erro desconhecido' }
      }));
    }
  }, []);
  ...
  const loadGamificationData = useCallback(() => {
    return Promise.all([
      ...
      loadData('/api/dashboard/popular-achievements?limit=5', 'popularAchievements')
    ]);
  }, [loadData]);
