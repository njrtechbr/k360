...
export function useRhConfigData() {
  ...
  const fetchCollection = useCallback(
    async (collectionName: "funcoes" | "setores") => {
      ...
      try {
        ...
        console.log(
          `PERF: fetchCollection for ${collectionName} (${list.length} items) took ${(endTime - startTime).toFixed(2)}ms`,
        );
        return list;
      } catch (error) {
        ...
        toast({
          variant: "destructive",
          title: "Erro Crítico",
          description: `Não foi possível carregar ${collectionName}.`,
        });
        return [];
      }
    },
  ...
  const addFuncao = async (funcao: string) => {
    try {
      ...
      toast({
        title: "Sucesso!",
        description: `A função "${funcao}" foi adicionada.`,
      });
    } catch (error) {
  ...
  const updateFuncao = async (oldFuncao: string, newFuncao: string) => {
    try {
      ...
      toast({
        title: "Sucesso!",
        description:
          "Função atualizada. Lembre-se de atualizar os atendentes, se necessário.",
      });
    } catch (error) {
    ...
    try {
      ...
      toast({
        ...
        description:
          error instanceof Error
            ? error.message
            : "Não foi possível remover a função.",
      });
      throw error;
    }
  };
  ...
  const addSetor = async (setor: string) => {
    try {
      ...
      toast({
        title: "Sucesso!",
        description: `O setor "${setor}" foi adicionado.`,
      });
    } catch (error) {
      ...
      toast({
        ...
        description:
          error instanceof Error
            ? error.message
            : "Não foi possível adicionar o setor.",
      });
      throw error;
    }
  };
  ...
  const updateSetor = async (oldSetor: string, newSetor: string) => {
    try {
      ...
      toast({
        title: "Sucesso!",
        description:
          "Setor atualizado. Lembre-se de atualizar os atendentes, se necessário.",
      });
    } catch (error) {
