...
import { AnalyticsService, AttendantsService, EvaluationsService } from '@/services/survey';
...
export function useEvaluationAnalytics({
  ...
  const filteredEvaluations = useMemo(() => {
    ...
    return evaluations.filter(evaluation => new Date(evaluation.data) >= startDate);
  ...
  const attendantsData = useMemo(() => {
    ...
    const attendantsPerformance = AttendantsService.getAttendantsPerformance(safeAttendants, filteredEvaluations);
    const topAttendants = AttendantsService.getTopRatedAttendants(safeAttendants, filteredEvaluations, 5);
    const lowPerformers = AttendantsService.getLowPerformanceAttendants(safeAttendants, filteredEvaluations, 3.5);
  ...
  const stats = useMemo((): EvaluationStats => {
    ...
    const basicStats = EvaluationsService.calculateBasicStats(filteredEvaluations);
    
    // Calcular tendência usando comparação de períodos
    const comparison = AnalyticsService.calculatePeriodComparison(filteredEvaluations, 14);
    ...
    const analyticsDistribution = AnalyticsService.calculateRatingDistribution(filteredEvaluations);
    analyticsDistribution.forEach(item => {
      ratingDistribution[item.rating as keyof typeof ratingDistribution] = item.count;
    });
  ...
  const sentimentByAttendant = useMemo(() => {
    const result = new Map<string, { positive: number; negative: number; neutral: number }>();
    
    analyses.forEach(analysis => {
      const evaluation = evaluations.find(evaluation => evaluation.id === analysis.evaluationId);
      if (evaluation) {
        const current = result.get(evaluation.attendantId) || { positive: 0, negative: 0, neutral: 0 };
  ...
  const getAttendantStats = useCallback((attendantId: string) => {
    const attendantEvals = EvaluationsService.filterByAttendant(filteredEvaluations, attendantId);
    ...
    const stats = AttendantsService.calculateAttendantStats(attendantId, attendantEvals);
    const sentiment = sentimentByAttendant.get(attendantId) || { positive: 0, negative: 0, neutral: 0 };
  ...
  return {
    ...
    recentTrends: [],
    sentimentByAttendant: Array.from(sentimentByAttendant.entries()).map(([attendantId, sentiment]) => {
      ...
      return {
        ...
        totalAnalyzed: sentiment.positive + sentiment.negative + sentiment.neutral
      };
    }),
