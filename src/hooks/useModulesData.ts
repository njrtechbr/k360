...
export function useModulesData() {
  ...
  const fetchModules = useCallback(async () => {
    ...
    try {
      ...
      console.log(
        `PERF: fetchModules (${modulesList.length} items) took ${(endTime - startTime).toFixed(2)}ms`,
      );
      return modulesList;
    } catch (error) {
  ...
  const addModule = useCallback(
    async (moduleData: Omit<Module, "id" | "active">) => {
      try {
        const response = await fetch("/api/modules", {
          method: "POST",
          headers: {
        ...
        toast({
          ...
          description: `O módulo "${newModule.name}" foi criado.`,
        });
        return newModule;
      } catch (error) {
        ...
        toast({
          ...
          description:
            error instanceof Error
              ? error.message
              : "Não foi possível adicionar o módulo.",
        });
        throw error;
      }
    },
  ...
  const updateModule = useCallback(
    async (
      moduleId: string,
      moduleData: Partial<Omit<Module, "id" | "active">>,
    ) => {
      try {
        const response = await fetch("/api/modules", {
          method: "PUT",
    ...
    async (moduleId: string) => {
      try {
        ...
        if (!response.ok) {
          const errorData = await response.json();
          throw new Error(
            errorData.error || "Erro ao alterar status do módulo",
          );
        }
