...
export function useXpNotifications() {
  ...
  const notifyXpGrant = useCallback(
      ...
      if (data.achievementsUnlocked && data.achievementsUnlocked.length > 0) {
        data.achievementsUnlocked.forEach((achievement, index) => {
          setTimeout(
            ...
            (index + 1) * 1000,
          ); // Escalonar notificações de conquistas
        });
      }
    },
  ...
  const notifyLevelUp = useCallback(
    (newLevel: number, xpAmount: number) => {
      addNotification({
        type: "success",
  ...
  useEffect(() => {
    ...
    const handleLevelUpEvent = (
      event: CustomEvent<{ level: number; xp: number }>,
    ) => {
    ...
    return () => {
      ...
      window.removeEventListener(
        "level-up",
        handleLevelUpEvent as EventListener,
      );
    };
