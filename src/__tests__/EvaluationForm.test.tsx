...
describe("EvaluationForm", () => {
  ...
  it("deve validar campos obrigatórios", async () => {
    ...
    const submitButton = screen.getByRole("button", {
      name: /salvar avaliação/i,
    });
    await user.click(submitButton);

    expect(screen.getByText("Selecione um atendente")).toBeInTheDocument();
    expect(
      screen.getByText("A nota deve ser entre 1 e 5 estrelas"),
    ).toBeInTheDocument();
  ...
  it("deve submeter formulário com dados válidos", async () => {
    ...
    const comentarioInput = screen.getByPlaceholderText(
      /descreva sua experiência/i,
    );
