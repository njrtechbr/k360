...
describe('Validadores de entidades específicas', () => {
  describe('isValidAttendant', () => {
    ...
    it('deve retornar false para attendant inválido', () => {
      ...
      expect(isValidAttendant({ ...validAttendant, id: 'invalid-uuid' })).toBe(false);
  ...
  describe('isValidEvaluation', () => {
    ...
    it('deve retornar false para evaluation inválida', () => {
      ...
      expect(isValidEvaluation({ ...validEvaluation, xpGained: -1 })).toBe(false);
    });
  });
  ...
  describe('isValidImportStatus', () => {
    ...
    it('deve retornar false para importStatus inválido', () => {
      ...
      expect(isValidImportStatus({ ...validImportStatus, isOpen: 'true' })).toBe(false);
      expect(isValidImportStatus({ ...validImportStatus, progress: -1 })).toBe(false);
      expect(isValidImportStatus({ ...validImportStatus, progress: 101 })).toBe(false);
      expect(isValidImportStatus({ ...validImportStatus, status: 'invalid' })).toBe(false);
    });
  });
});
...
describe('Funções de validação segura', () => {
  describe('validateAttendantArray', () => {
    ...
    it('deve retornar array vazio para dados null/undefined', () => {
      ...
      expect(resultNull.errors).toContain('Dados de atendentes são null ou undefined');
      ...
      expect(resultUndefined.errors).toContain('Dados de atendentes são null ou undefined');
    });
