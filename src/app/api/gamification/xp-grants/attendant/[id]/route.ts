...
export async function GET(
  ...
  try {
    ...
    const sortBy = searchParams.get('sortBy') as 'grantedAt' | 'points' | 'typeName' | 'granterName' || 'grantedAt';
    const sortOrder = searchParams.get('sortOrder') as 'asc' | 'desc' || 'desc';

    // Validar parâmetros de paginação
    if (page < 1 || limit < 1 || limit > 200) {
      return NextResponse.json(
        { error: 'Parâmetros de paginação inválidos. Page >= 1, Limit entre 1 e 200' },
        { status: 400 }
      );
    }
    ...
    if (!validSortBy.includes(sortBy)) {
      return NextResponse.json(
        { 
          error: 'Parâmetro sortBy inválido. Valores válidos: grantedAt, points, typeName, granterName',
    ...
    const grants = await XpAvulsoService.findGrantsByAttendantWithSort(attendantId, sortBy, sortOrder);
    ...
    const grantsByType = grants.reduce((acc, grant) => {
    ...
    const responseData: any = {
      ...
      summary: {
        totalGrants,
        totalPoints,
        averagePoints: totalGrants > 0 ? Math.round(totalPoints / totalGrants) : 0
      },
