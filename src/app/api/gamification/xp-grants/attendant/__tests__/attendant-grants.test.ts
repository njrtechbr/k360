...
describe('/api/gamification/xp-grants/attendant/[id]', () => {
  ...
  describe('GET - Buscar concessões por atendente', () => {
    it('deve buscar concessões do atendente com sucesso', async () => {
      ...
      mockXpAvulsoService.findGrantsByAttendantWithSort.mockResolvedValue(mockGrants as any);

      // Criar request mock
      const request = new NextRequest('http://localhost/api/gamification/xp-grants/attendant/att-1');
    ...
    it('deve retornar dados vazios para atendente sem concessões', async () => {
      ...
      const request = new NextRequest('http://localhost/api/gamification/xp-grants/attendant/att-2');
    ...
    it('deve retornar erro 400 para ID inválido', async () => {
      ...
      const request = new NextRequest('http://localhost/api/gamification/xp-grants/attendant/');

      const response = await GET(request, { params: { id: '' } });
      const data = await response.json();

      expect(response.status).toBe(400);
      expect(data.error).toBe('ID do atendente é obrigatório e deve ser uma string válida');
    });
    ...
    it('deve retornar erro 404 para atendente não encontrado', async () => {
      ...
      const request = new NextRequest('http://localhost/api/gamification/xp-grants/attendant/invalid-id');
    ...
    it('deve aplicar ordenação customizável por pontos', async () => {
      ...
      const request = new NextRequest('http://localhost/api/gamification/xp-grants/attendant/att-1?sortBy=points&sortOrder=asc');

      const response = await GET(request, { params: { id: 'att-1' } });
      const data = await response.json();

      expect(response.status).toBe(200);
      expect(mockXpAvulsoService.findGrantsByAttendantWithSort).toHaveBeenCalledWith('att-1', 'points', 'asc');
    });
    ...
    it('deve retornar erro para parâmetros de ordenação inválidos', async () => {
      ...
      const request = new NextRequest('http://localhost/api/gamification/xp-grants/attendant/att-1?sortBy=invalid&sortOrder=wrong');
