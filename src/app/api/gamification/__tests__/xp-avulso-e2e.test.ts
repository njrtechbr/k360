...
describe('XP Avulso - Testes E2E Completos', () => {
  ...
  describe('Fluxo Completo: Criação → Concessão → Verificação', () => {
    it('deve executar fluxo completo de criação de tipo até concessão com conquistas', async () => {
      ...
      const mockGrant = {
        ...
        xpEvent: {
          ...
          attendantId: 'att-joao',
          points: 150,
          reason: 'XP Avulso: Excelência no Atendimento - Resolveu problema complexo com excelência'
        },
      ...
      expect(grantXpData.data.justification).toBe('Resolveu problema complexo com excelência');

      // Verificar conquistas desbloqueadas
      expect(grantXpData.data.notification).toBeDefined();
      expect(grantXpData.data.notification.achievementsUnlocked).toHaveLength(2);
      expect(grantXpData.data.notification.achievementsUnlocked[0].title).toBe('Primeiro Reconhecimento');
      expect(grantXpData.data.notification.achievementsUnlocked[1].title).toBe('Excelência Reconhecida');
      ...
      mockXpAvulsoService.findGrantHistory.mockResolvedValue(mockHistoryResult as any);

      const historyRequest = new NextRequest('http://localhost/api/gamification/xp-grants?page=1&limit=20');
      ...
      const attendantGrantsRequest = new NextRequest('http://localhost/api/gamification/xp-grants/attendant/att-joao');
      const attendantGrantsResponse = await getAttendantGrants(attendantGrantsRequest, { params: { id: 'att-joao' } });
      ...
      expect(mockXpAvulsoService.findGrantsByAttendantWithSort).toHaveBeenCalledWith(
        'att-joao',
        'grantedAt',
        'desc'
      );
      ...
      expect(mockAuditLogger.logAdminAction).toHaveBeenNthCalledWith(1,
        'CREATE_XP_TYPE',
        'admin-1',
      ...
      expect(mockAuditLogger.logAdminAction).toHaveBeenNthCalledWith(2,
        'admin-1',
        'XP_GRANT_CREATED',
        expect.objectContaining({
    ...
    it('deve testar experiência do atendente com mudança de nível', async () => {
      ...
      const levelUpAchievement = grantData.data.notification.achievementsUnlocked.find(
        (a: any) => a.id === 'achievement-level-up'
      );
      expect(levelUpAchievement).toBeDefined();
      expect(levelUpAchievement.title).toBe('Subiu de Nível!');

      const customerLoveAchievement = grantData.data.notification.achievementsUnlocked.find(
    ...
    it('deve verificar integração com sistema de conquistas', async () => {
      ...
      expect(achievements.find((a: any) => a.id === 'achievement-xp-based')).toBeDefined();
      expect(achievements.find((a: any) => a.id === 'achievement-grant-count')).toBeDefined();
      expect(achievements.find((a: any) => a.id === 'achievement-type-specific')).toBeDefined();
    });
    ...
    it('deve processar múltiplas concessões e verificar acumulação', async () => {
      ...
      const mockGrant1 = {
        ...
        notificationData: {
          ...
          levelUp: null,
          achievementsUnlocked: [
            { id: 'achievement-innovator', title: 'Inovador', description: 'Primeira sugestão aceita' }
          ]
        }
      };
      ...
      const mockGrant2 = {
        ...
        notificationData: {
          ...
          levelUp: null,
          achievementsUnlocked: [
            { id: 'achievement-team-player', title: 'Jogador de Equipe', description: 'Excelente colaboração' }
          ]
        }
      };
      ...
      // Primeira concessão
      const grant1Request = new NextRequest('http://localhost/api/gamification/xp-grants', {
        method: 'POST',
        body: JSON.stringify({
          attendantId: 'att-maria',
          typeId: 'type-initiative',
      ...
      // Segunda concessão
      const grant2Request = new NextRequest('http://localhost/api/gamification/xp-grants', {
        method: 'POST',
        body: JSON.stringify({
          attendantId: 'att-maria',
          typeId: 'type-teamwork',
      ...
      mockXpAvulsoService.findGrantsByAttendantWithSort.mockResolvedValue([mockGrant2, mockGrant1] as any);

      const consolidatedRequest = new NextRequest('http://localhost/api/gamification/xp-grants/attendant/att-maria');
      const consolidatedResponse = await getAttendantGrants(consolidatedRequest, { params: { id: 'att-maria' } });
      ...
      expect(consolidatedData.data.grants[0].type.name).toBe('Trabalho em Equipe');
  ...
  describe('Testes de Performance e Escalabilidade', () => {
    it('deve lidar com histórico grande com paginação eficiente', async () => {
      ...
      mockXpAvulsoService.findGrantHistory.mockResolvedValue(mockLargeHistory as any);
