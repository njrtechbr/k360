...
describe('XP Avulso - Testes Avançados de Segurança', () => {
  ...
  describe('Controle de Acesso Granular por Role', () => {
    describe('SUPERADMIN - Acesso Total', () => {
        ...
        const operations = [
          {
            ...
            request: () => getXpTypes(new NextRequest('http://localhost/api/gamification/xp-types')),
            expectedStatus: 200
          },
          {
          ...
          },
          {
            ...
            request: () => getXpGrants(new NextRequest('http://localhost/api/gamification/xp-grants')),
            expectedStatus: 200
          },
          {
            name: 'Ver estatísticas',
            request: () => getStatistics(new NextRequest('http://localhost/api/gamification/xp-grants/statistics')),
            expectedStatus: 200
          },
          {
            name: 'Ver estatísticas diárias',
            request: () => getDailyStats(new NextRequest('http://localhost/api/gamification/xp-grants/daily-stats')),
            expectedStatus: 200
          }
        ];
      ...
      it('deve permitir acesso a dados sensíveis para SUPERADMIN', async () => {
        const sensitiveStatistics = {
          ...
          totalPoints: 50000,
          grantsByGranter: [
            { granterId: 'admin-1', granterName: 'Admin 1', count: 500, totalPoints: 25000 },
            { granterId: 'admin-2', granterName: 'Admin 2', count: 500, totalPoints: 25000 }
          ],
          suspiciousActivity: [
            { granterId: 'admin-3', reason: 'Muitas concessões em pouco tempo', count: 100 }
          ]
        };

        mockXpAvulsoService.getGrantStatistics.mockResolvedValue(sensitiveStatistics as any);

        const request = new NextRequest('http://localhost/api/gamification/xp-grants/statistics?includeSensitive=true');
      ...
      it('deve permitir operações operacionais para ADMIN', async () => {
        ...
        const allowedOperations = [
          ...
          },
          {
            ...
            request: () => getXpGrants(new NextRequest('http://localhost/api/gamification/xp-grants?granterId=admin-1'))
          }
        ];
    ...
    describe('USUARIO - Acesso Negado', () => {
      ...
      it('deve negar todas as operações para USUARIO', async () => {
        const allOperations = [
          () => getXpTypes(new NextRequest('http://localhost/api/gamification/xp-types')),
          () => postXpTypes(new NextRequest('http://localhost/api/gamification/xp-types', { method: 'POST' })),
          () => getXpGrants(new NextRequest('http://localhost/api/gamification/xp-grants')),
          () => postXpGrants(new NextRequest('http://localhost/api/gamification/xp-grants', { method: 'POST' })),
          () => getStatistics(new NextRequest('http://localhost/api/gamification/xp-grants/statistics'))
        ];
  ...
  describe('Testes de Limites de Concessão e Auditoria', () => {
    ...
    it('deve detectar padrões suspeitos de concessão', async () => {
      ...
      expect(data.error).toBe('Padrão suspeito detectado: muitas concessões para o mesmo atendente');
    ...
    it('deve validar limites de concessão por atendente', async () => {
      ...
      mockXpAvulsoService.grantXp.mockRejectedValue(
        new Error('Atendente já recebeu o limite máximo de XP avulso hoje (300 pontos)')
      );
      ...
      expect(response.status).toBe(429);
      expect(data.error).toBe('Atendente já recebeu o limite máximo de XP avulso hoje (300 pontos)');
    });
  });
  ...
  describe('Testes de Validação de Dados Avançada', () => {
    ...
    it('deve validar e sanitizar campos de texto contra XSS', async () => {
      ...
      for (const maliciousInput of maliciousInputs) {
        ...
        expect(data.details).toContain('Conteúdo potencialmente perigoso detectado');
      }
    });
    ...
    it('deve validar limites de tamanho de campos', async () => {
      ...
      for (const input of oversizedInputs) {
        ...
        if (input.field === 'justification') {
          request = new NextRequest('http://localhost/api/gamification/xp-grants', {
            method: 'POST',
            body: JSON.stringify({
              attendantId: 'att-1',
              typeId: 'type-1',
          ...
          const body: any = {
            ...
            description: input.field === 'description' ? input.value : 'Descrição válida',
            points: 50
          };
          
          request = new NextRequest('http://localhost/api/gamification/xp-types', {
            method: 'POST',
        ...
        const response = input.field === 'justification' 
          ? await postXpGrants(request)
    ...
    it('deve validar tipos de dados e formatos', async () => {
      const invalidDataTypes = [
        ...
        {
          name: 'Cor inválida',
          data: { name: 'Test', description: 'Test', points: 50, color: 'not-a-color' },
          expectedError: 'Cor deve estar no formato hexadecimal'
        },
        {
          name: 'Categoria inválida',
          data: { name: 'Test', description: 'Test', points: 50, category: 'categoria_inexistente' },
  ...
  describe('Testes de Proteção contra Ataques Avançados', () => {
    ...
    it('deve proteger contra ataques de enumeração de usuários', async () => {
      ...
      const testIds = [
        'att-1', 'att-2', 'att-999', 'att-nonexistent',
        'user-1', 'admin-1', 'invalid-id'
      ];
    ...
    it('deve detectar e bloquear tentativas de bypass de autenticação', async () => {
      ...
      for (const attempt of bypassAttempts) {
        ...
        expect(response.status).toBe(403);
        expect(data.error).toBe('Tentativa de bypass de autenticação detectada');
      }
    });
  });
});