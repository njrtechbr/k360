...
describe("XP Avulso - Integração Final", () => {
  ...
  describe("Rankings e Temporadas", () => {
    it("deve incluir XP avulso nos rankings de temporada", async () => {
      ...
      // Calcular ranking
      const rankings =
        await GamificationService.calculateSeasonRankings(testSeasonId);
    ...
    it("deve filtrar XP por temporada corretamente", async () => {
      ...
      const xpTemporadaAtiva = await GamificationService.calculateTotalXp(
        testAttendantId,
        testSeasonId,
      );
      const xpTemporadaInativa = await GamificationService.calculateTotalXp(
        testAttendantId,
        season2.id,
      );
  ...
  describe("Conquistas", () => {
    ...
    it("deve verificar conquistas após concessão de XP avulso", async () => {
      ...
      const unlockedAchievements =
        await GamificationService.findUnlockedAchievements(testAttendantId);
    ...
    it("deve considerar XP avulso no cálculo total para conquistas", async () => {
      ...
      // Verificar XP total
      const totalXp =
        await GamificationService.calculateTotalXp(testAttendantId);
  ...
  describe("Compatibilidade com Sistema Existente", () => {
    ...
    it("deve aparecer corretamente no histórico de eventos XP", async () => {
      ...
      const xpEvents =
        await GamificationService.findXpEventsByAttendant(testAttendantId);
      ...
      expect(xpEvents[0].reason).toContain(
        "XP Avulso: Excelência no Atendimento",
      );
    });
