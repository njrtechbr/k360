  ...
  try {
    ...
    if (config) {
      ...
      const achievementsFromDb = achievementConfigs.map(config => ({
        ...
        icon: INITIAL_ACHIEVEMENTS.find(a => a.id === config.id)?.icon || '🏆',
        color: INITIAL_ACHIEVEMENTS.find(a => a.id === config.id)?.color || 'bg-yellow-500',
        isUnlocked: false
      }));
      ...
      const levelRewardsFromDb = levelTrackConfigs.map(config => ({
        ...
        xp: config.xp,
        active: config.active,
        icon: INITIAL_LEVEL_REWARDS.find(r => r.level === config.level)?.icon || '🎯'
      }));
      
      const mergedAchievements = achievementsFromDb.length > 0 ? 
        mergeAchievementsWithDefaults(achievementsFromDb) : 
        INITIAL_ACHIEVEMENTS;
      
      const mergedLevelRewards = levelRewardsFromDb.length > 0 ? 
      ...
      if (achievementConfigs.length === 0) {
        await prisma.achievementConfig.createMany({
          data: INITIAL_ACHIEVEMENTS.map(({ isUnlocked, icon, color, ...ach }) => ({
            ...ach,
            active: true
          }))
        });
      ...
      const gamificationConfig = {
        ...newConfig,
        ratingScores,
        achievements: achievementsFromDb.length > 0 ? achievementsFromDb : INITIAL_ACHIEVEMENTS,
        levelRewards: levelRewardsFromDb.length > 0 ? levelRewardsFromDb : INITIAL_LEVEL_REWARDS,
        seasons,
      };
