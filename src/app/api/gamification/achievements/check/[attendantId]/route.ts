...
export async function GET(
  ...
  try {
    ...
    const achievementStatus = await AchievementCheckerService.getAttendantAchievementStatus(attendantId);
    ...
    return NextResponse.json({
      ...
      summary: {
        ...
        progress: achievementStatus.length > 0 
          ? Math.round((achievementStatus.filter(a => a.isUnlocked).length / achievementStatus.length) * 100)
          : 0
      }
    });
  } catch (error) {
...
export async function POST(
  ...
  try {
    ...
    return NextResponse.json({
      ...
      message: achievementResults.newAchievements.length > 0 
