...
export async function GET(request: NextRequest) {
  try {
    ...
    if (includeStats) {
      ...
      const deptStats = departmentStats.reduce(
        (acc, stat) => {
          const attendant = attendantsWithDept.find(
            (a) => a.id === stat.attendantId,
          );
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (seasonIds && seasonIds.length > 0) {
      ...
      if (seasons.length > 0) {
        const minStartDate = Math.min(
          ...seasons.map((s) => s.startDate.getTime()),
        );
    ...
    if (groupBy === "attendant") {
      ...
      const levelData = xpData.reduce(
        (acc, xp) => {
          const level = LeaderboardService.calculateLevel(
            xp._sum.xpGained || 0,
          );
      ...
      const levelLeaderboard = Object.values(levelData)
        .map((lvl: any) => ({
          ...lvl,
          averageXp:
            lvl.participantCount > 0 ? lvl.totalXp / lvl.participantCount : 0,
        }))
