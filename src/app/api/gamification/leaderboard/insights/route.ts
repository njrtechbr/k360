  ...
  try {
    ...
    const generalInsights = {
      ...
      averageXpPerParticipant:
        totalXps.length > 0
          ? totalXps.reduce((a, b) => a + b, 0) / totalXps.length
          : 0,
      medianXp:
        totalXps.length > 0
          ? totalXps.sort((a, b) => a - b)[Math.floor(totalXps.length / 2)]
          : 0,
      topPerformerXp: Math.max(...totalXps, 0),
      xpDistribution: {
        q1:
          totalXps.length > 0
            ? totalXps.sort((a, b) => a - b)[Math.floor(totalXps.length * 0.25)]
            : 0,
        q2:
          totalXps.length > 0
            ? totalXps.sort((a, b) => a - b)[Math.floor(totalXps.length * 0.5)]
            : 0,
        q3:
          totalXps.length > 0
            ? totalXps.sort((a, b) => a - b)[Math.floor(totalXps.length * 0.75)]
            : 0,
      },
    };

    // Insights por departamento
    const departmentInsights = leaderboardData.reduce(
      (acc, item) => {
    ...
    Object.values(departmentInsights).forEach((dept: any) => {
      dept.averageXp =
        dept.participantCount > 0 ? dept.totalXp / dept.participantCount : 0;
    });

    // Insights de performance
    const performanceInsights = {
      highPerformers: leaderboardData.slice(
        0,
        Math.ceil(leaderboardData.length * 0.1),
      ), // Top 10%
      lowPerformers: leaderboardData.slice(
        -Math.ceil(leaderboardData.length * 0.1),
      ), // Bottom 10%
      ...
      performanceGaps: {
        ...
        topToAverage:
          Math.max(...totalXps, 0) - generalInsights.averageXpPerParticipant,
        averageToBottom:
          generalInsights.averageXpPerParticipant - Math.min(...totalXps, 0),
      },
    };
    ...
    if (includeComparisons && period === "current") {
      ...
      if (currentSeason) {
        ...
        if (previousSeason) {
          ...
          const previousTotalXp = previousXpData.reduce(
            (sum, x) => sum + (x._sum.points || 0),
            0,
          );
          const previousParticipants = previousXpData.length;
          const previousAverageXp =
            previousParticipants > 0
              ? previousTotalXp / previousParticipants
              : 0;
          ...
          result.comparisons = {
            ...
            changes: {
              ...
              participantChange:
                generalInsights.totalParticipants - previousParticipants,
              averageXpChange:
                generalInsights.averageXpPerParticipant - previousAverageXp,
              totalXpChangePercent:
                previousTotalXp > 0
                  ? ((generalInsights.totalXpAwarded - previousTotalXp) /
                      previousTotalXp) *
                    100
                  : 0,
              participantChangePercent:
                previousParticipants > 0
                  ? ((generalInsights.totalParticipants -
    ...
    if (includeTrends) {
      ...
      const weeklyData = await prisma.$queryRaw`
        ...
        ${
          department
            ? prisma.$queryRaw`AND "attendantId" IN (
    ...
    if (includeRecommendations) {
      ...
      const avgParticipation =
        deptParticipation.reduce((sum, d) => sum + d.participantCount, 0) /
        deptParticipation.length;

      const lowParticipationDepts = deptParticipation.filter(
        (d) => d.participantCount < avgParticipation * 0.7,
      );
      ...
      const levelCounts = leaderboardData.reduce(
        (acc, item) => {
          if (!item) return acc;
