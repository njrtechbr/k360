...
export async function GET(request: NextRequest) {
  try {
    ...
    const totalEvaluations =
      totalEvaluationsResult.status === "fulfilled"
        ? totalEvaluationsResult.value
        : 0;
    const totalAttendants =
      totalAttendantsResult.status === "fulfilled"
        ? totalAttendantsResult.value
        : 0;
    const averageRating =
      averageRatingResult.status === "fulfilled"
        ? averageRatingResult.value._avg.nota || 0
        : 0;
    const activeSeasons =
      activeSeasonsResult.status === "fulfilled"
        ? activeSeasonsResult.value
        : 0;
    ...
    return NextResponse.json(
      ...
      { status: 200 },
    ); // Não retornar 500 para não quebrar o dashboard
  }
}
