...
export async function GET(request: NextRequest, { params }: RouteParams) {
  ...
  if (!securityResult.success || !securityResult.user) {
    return (
      ...
      new NextResponse("Acesso negado", { status: 401 })
    );
  }
  ...
  try {
    ...
    try {
      ...
      if (fileStats.size !== backupInfo.size) {
        console.error(
          `[BACKUP_INTEGRITY_ERROR] Tamanho do arquivo não confere: esperado ${backupInfo.size}, encontrado ${fileStats.size}`,
        );
        return NextResponse.json(
      ...
      console.error(
        `[BACKUP_FILE_ERROR] Arquivo não encontrado: ${backupInfo.filepath}`,
        error,
      );
      return NextResponse.json(
    ...
    try {
      ...
      if (!isValid) {
        console.error(
          `[BACKUP_INTEGRITY_ERROR] Falha na validação de integridade: ${backupInfo.filepath}`,
        );
        return NextResponse.json(
      ...
      console.error(
        `[BACKUP_VALIDATION_ERROR] Erro ao validar backup: ${backupInfo.filepath}`,
        error,
      );
      return NextResponse.json(
    ...
    if (backupInfo.size < 50 * 1024 * 1024) {
      try {
        ...
        console.log(
          `[BACKUP_AUDIT] Download direto concluído para backup ${backupInfo.id} - ${backupInfo.filename}`,
        );
        ...
        console.error(
          `[BACKUP_READ_ERROR] Erro ao ler arquivo: ${backupInfo.filepath}`,
          error,
        );
        return NextResponse.json(
    ...
    try {
      ...
      const webStream = new ReadableStream({
        start(controller) {
          ...
          stream.on("error", (error) => {
            console.error(
              `[BACKUP_STREAM_ERROR] Erro no streaming: ${backupInfo.filepath}`,
              error,
            );
      ...
      console.log(
        `[BACKUP_AUDIT] Streaming iniciado para backup ${backupInfo.id} - ${backupInfo.filename}`,
      );
      ...
      console.error(
        `[BACKUP_STREAM_ERROR] Erro ao criar stream: ${backupInfo.filepath}`,
        error,
      );
      return NextResponse.json(
