...
describe("Backup File Operations Integration Tests", () => {
  ...
  beforeAll(async () => {
    ...
    fs.writeFileSync(
      path.join(testBackupDir, "small_backup.sql"),
      smallBackupContent,
    );
    fs.writeFileSync(
    ...
    fs.writeFileSync(
      path.join(testBackupDir, "large_backup.sql"),
      largeBackupContent,
    );

    // Criar arquivo corrompido
    fs.writeFileSync(
      path.join(testBackupDir, "corrupted_backup.sql"),
      "INVALID SQL CONTENT",
    );

    // Criar arquivo comprimido simulado
    const compressedContent =
      Buffer.from(smallBackupContent).toString("base64");
    fs.writeFileSync(
      path.join(testBackupDir, "compressed_backup.sql.gz"),
      compressedContent,
    );
  });
  ...
  describe("Download de Arquivos", () => {
    it("deve fazer download de arquivo pequeno corretamente", async () => {
      ...
      const response = await GET(
        new NextRequest(
          "http://localhost:3000/api/backup/download/small-backup",
        ),
    ...
    it("deve fazer streaming de arquivo médio eficientemente", async () => {
      ...
      const response = await GET(
        new NextRequest(
          "http://localhost:3000/api/backup/download/medium-backup",
        ),
      ...
      expect(response.headers.get("Content-Length")).toBe(
        mediumBackupContent.length.toString(),
      );
    ...
    it("deve fazer streaming de arquivo grande com headers apropriados", async () => {
      ...
      const response = await GET(
        new NextRequest(
          "http://localhost:3000/api/backup/download/large-backup",
        ),
      ...
      expect(response.headers.get("Content-Type")).toBe("application/sql");
      expect(response.headers.get("Content-Length")).toBe(
        largeBackupContent.length.toString(),
      );
      expect(response.headers.get("Accept-Ranges")).toBe("bytes");

      // Para arquivos grandes, verificar apenas o início do conteúdo
      const downloadedContent = await response.text();
      expect(downloadedContent.startsWith("SELECT * FROM large_table;")).toBe(
        true,
      );
    });
    ...
    it("deve detectar e rejeitar arquivos corrompidos", async () => {
      ...
      const response = await GET(
        new NextRequest(
          "http://localhost:3000/api/backup/download/corrupted-backup",
        ),
    ...
    it("deve lidar com arquivos inexistentes graciosamente", async () => {
      ...
      const response = await GET(
        new NextRequest(
          "http://localhost:3000/api/backup/download/missing-backup",
        ),
  ...
  describe("Validação de Integridade de Arquivos", () => {
    it("deve calcular checksum MD5 corretamente", async () => {
      ...
      const { BackupService } = require("@/services/backupService");
      BackupService.calculateChecksum = jest
        .fn()
        .mockReturnValue(expectedChecksum);
    ...
    it("deve validar integridade usando checksum SHA-256", async () => {
      ...
      const { BackupService } = require("@/services/backupService");
      BackupService.validateChecksum = jest
        .fn()
        .mockImplementation((filePath, expectedChecksum, algorithm) => {
          if (algorithm === "sha256") {
  ...
  describe("Compressão de Arquivos", () => {
    it("deve lidar com arquivos comprimidos no download", async () => {
      ...
      const response = await GET(
        new NextRequest(
          "http://localhost:3000/api/backup/download/compressed-backup",
        ),
  ...
  describe("Segurança de Arquivos", () => {
    it("deve prevenir acesso a arquivos fora do diretório de backup", async () => {
      ...
      for (const maliciousPath of maliciousPaths) {
        ...
        const response = await GET(
          new NextRequest(
            "http://localhost:3000/api/backup/download/malicious",
          ),
          { params: { id: "malicious" } },
        );
