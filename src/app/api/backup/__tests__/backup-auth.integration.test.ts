...
describe('Backup API Authentication & Authorization Tests', () => {
  ...
  describe('Validação de Sessão', () => {
    it('deve retornar 401 para usuário não autenticado em todos os endpoints', async () => {
      ...
      const endpoints = [
        { module: '../create/route', method: 'POST', path: '/api/backup/create' },
        { module: '../list/route', method: 'GET', path: '/api/backup/list' },
        { module: '../download/[id]/route', method: 'GET', path: '/api/backup/download/test-id', params: { id: 'test-id' } },
        { module: '../[id]/route', method: 'DELETE', path: '/api/backup/test-id', params: { id: 'test-id' } },
        { module: '../status/[id]/route', method: 'GET', path: '/api/backup/status/test-id', params: { id: 'test-id' } }
      ];
      ...
      for (const endpoint of endpoints) {
        ...
        const response = await handler(request, endpoint.params ? { params: endpoint.params } : undefined);
  ...
  describe('Validação de Parâmetros de Segurança', () => {
    ...
    it('deve validar ID de backup para prevenir path traversal', async () => {
      ...
      for (const maliciousId of maliciousIds) {
        const { GET } = await import('../download/[id]/route');
        const response = await GET(
          new NextRequest(`http://localhost:3000/api/backup/download/${encodeURIComponent(maliciousId)}`),
  ...
  describe('Rate Limiting e Proteção contra Abuso', () => {
    ...
    it('deve implementar rate limiting para criação de backups', async () => {
      ...
      const requests = Array.from({ length: 10 }, () => 
        POST(new NextRequest('http://localhost:3000/api/backup/create', {
          method: 'POST',
