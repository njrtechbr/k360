...
const mockBackupValidator = require('@/services/backupValidator').BackupValidator;
...
describe('GET /api/backup/download/[id]', () => {
    ...
    it('deve retornar 401 se usuário não estiver autenticado', async () => {
        ...
        const request = new NextRequest('http://localhost/api/backup/download/test-id');
    ...
    it('deve retornar 403 se usuário não tiver permissões adequadas', async () => {
        ...
        expect(data.error).toBe('Permissões insuficientes para fazer download de backups');
    });
    ...
    it('deve permitir download para usuários com permissões adequadas', async () => {
        ...
        for (const role of roles) {
            ...
            expect(response.headers.get('Content-Disposition')).toBe('attachment; filename="test.sql"');
        }
    });
    ...
    it('deve configurar headers corretos para download', async () => {
        ...
        expect(response.status).toBe(200);
        expect(response.headers.get('Content-Type')).toBe('application/octet-stream');
        expect(response.headers.get('Content-Disposition')).toBe('attachment; filename="backup_2025-01-09.sql"');
    ...
    it('deve usar streaming para arquivos grandes', async () => {
        ...
        expect(response.status).toBe(200);
        expect(mockFs.createReadStream).toHaveBeenCalledWith('/path/large_backup.sql');
    });
});