  ...
  try {
    ...
    await prisma.xpEvent.create({
      data: {
        ...
        multiplier: activeSeason ? gamificationConfig.globalXpMultiplier * activeSeason.xpMultiplier : gamificationConfig.globalXpMultiplier,
    ...
    try {
      ...
      if (achievementResults.newAchievements.length > 0) {
        console.log(`🏆 ${achievementResults.newAchievements.length} nova(s) conquista(s) desbloqueada(s) para atendente ${finalEvaluation.attendantId}`);
      }
    } catch (error) {
