...
export default function GerenciarAtendentesPage() {
  ...
  const { attendants, deleteAttendants } = useApi();
  ...
  const sortedAttendants = attendants.data
    ? [...attendants.data].sort((a, b) => a.name.localeCompare(b.name))
    : [];
  ...
  const handleDeleteSelected = async () => {
    try {
      await deleteAttendants.mutate({
        attendantIds: selectedIds,
        title: "Atendentes selecionados",
      });
      setSelectedIds([]);
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start gap-4">
          <div>
            ...
            <p className="text-muted-foreground">
              Selecione e exclua atendentes específicos ou apague todos os
                  ...
                  <TableHead className="w-[50px]">
                    <Checkbox
                      checked={
                        ...
                        selectedIds.length === sortedAttendants.length
                      }
                ...
                {sortedAttendants.length > 0 ? (
                  sortedAttendants.map((attendant) => (
                    <TableRow
                      key={attendant.id}
                      data-state={
                        selectedIds.includes(attendant.id) && "selected"
                      }
                    >
                      <TableCell>
                        <Checkbox
                          ...
                          onCheckedChange={(checked) =>
                            handleSelectRow(attendant.id, !!checked)
                          }
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle className="flex items-center gap-2">
              ...
              Excluir Atendentes Selecionados?
            </AlertDialogTitle>
            <AlertDialogDescription>
              Você tem certeza que deseja excluir permanentemente os{" "}
              <strong>{selectedIds.length}</strong> atendentes selecionados?
              Esta ação não pode ser desfeita e todos os dados associados
              (avaliações, etc.) serão perdidos.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
      ...
      <AlertDialog open={isDeleteAllOpen} onOpenChange={setIsDeleteAllOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle className="flex items-center gap-2">
              <AlertCircle className="text-destructive" />
              Excluir TODOS os Atendentes?
            </AlertDialogTitle>
            <AlertDialogDescription>
              Você tem certeza absoluta? Esta ação excluirá permanentemente{" "}
              <strong>TODOS os {sortedAttendants.length}</strong> atendentes do
              sistema, assim como todas as suas avaliações. Esta ação não pode
              ser desfeita.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
