...
export default function RHConfiguracoesPage() {
  ...
  async function onSubmit(values: z.infer<typeof formSchema>) {
    if (!selectedItem) {
      ...
      if (selectedItem.type === "funcao")
        await updateFuncao(selectedItem.value, values.name);
        ...
        <div className="flex justify-between items-start">
          <div>
            <CardTitle className="capitalize">
              {type === "funcao" ? "Funções" : "Setores"}
            </CardTitle>
            <CardDescription>
              Gerencie as {type === "funcao" ? "funções" : "setores"}{" "}
              disponíveis no sistema.
            </CardDescription>
          </div>
          <Button onClick={() => handleAddClick(type)}>
            Adicionar {type === "funcao" ? "Função" : "Setor"}
          </Button>
        </div>
      </CardHeader>
      <CardContent>
        ...
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            <DialogTitle className="capitalize">
              {selectedItem ? "Editar" : "Adicionar"} {itemType}
            </DialogTitle>
          </DialogHeader>
          <Form {...form}>
            <form
            ...
            >
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    <FormLabel>
                      Nome d{itemType === "funcao" ? "a Função" : "o Setor"}
                    </FormLabel>
                    <FormControl>
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              ...
              item "{selectedItem?.value}". Se este item estiver em uso por
              algum atendente, pode ser necessário atualizar o cadastro dele
              manualmente.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
