...
const AchievementCard = ({
  ...
  <Card
    className={cn(
      ...
      unlockedAt ? "border-yellow-200 bg-yellow-50/50" : "opacity-60",
    )}
  >
      ...
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1">
          ...
          <div className="flex items-center justify-between">
            <Badge
              variant={unlockedAt ? "default" : "secondary"}
              className="text-xs"
            >
              +{achievement.xp} XP
            </Badge>
  ...
  <TooltipProvider>
    <Tooltip>
      <TooltipTrigger asChild>
        ...
        >
          <span
            className={cn("text-lg", unlockedAt ? "grayscale-0" : "grayscale")}
          >
            {achievement.icon}
          </span>
          {unlockedAt && (
...
export default function AttendantProfilePage() {
  ...
  const { attendants, evaluations, xpEvents, seasons, isAnyLoading } = useApi();
  ...
  const attendant = useMemo(() => {
    ...
    if (!attendants.data || !Array.isArray(attendants.data)) {
      console.warn(
        "AttendantProfilePage: attendants é null, undefined ou não é um array",
        attendants.data,
      );
      return undefined;
    }
  ...
  const stats = useMemo(() => {
    ...
    if (!attendant) {
      console.warn(
        "AttendantProfilePage: attendant não encontrado ou inválido",
      );
      return {
        totalXp: 0,
        totalEvaluations: 0,
    ...
    const safeXpEvents = Array.isArray(xpEvents.data) ? xpEvents.data : [];
    const safeEvaluations = Array.isArray(evaluations.data)
      ? evaluations.data
      : [];
    const safeSeasons = Array.isArray(seasons.data) ? seasons.data : [];
    ...
    if (
      ...
      console.info(
        "AttendantProfilePage: nenhum dado de gamificação disponível ainda",
      );
    }

    // XP total com validação segura
    const attendantXpEvents = safeXpEvents.filter(
      (e) => e && e.attendantId === id,
    );
    const totalXp = attendantXpEvents.reduce(
      (sum, e) => sum + (e.points || 0),
      0,
    );

    // Avaliações do atendente com validação segura
    const attendantEvaluations = safeEvaluations.filter(
      (e) => e && e.attendantId === id,
    );
    const totalEvaluations = attendantEvaluations.length;
    const averageRating =
      totalEvaluations > 0
        ? attendantEvaluations.reduce((sum, e) => sum + (e.nota || 0), 0) /
          totalEvaluations
        : 0;
    ...
    const now = new Date();
    const sortedSeasons = [...safeSeasons].sort(
      (a, b) =>
    ...
    if (currentSeason) {
      ...
      currentSeasonXp = attendantXpEvents
        .filter((e) => {
          const eventDate = new Date(e.date);
          return (
            eventDate >= currentSeasonStart && eventDate <= currentSeasonEnd
          );
        })
    ...
    sortedSeasons.forEach((season) => {
      ...
      const seasonRatingDist = seasonEvals.reduce(
        (acc, e) => {
          acc[e.nota as keyof typeof acc]++;
          return acc;
        },
      ...
      const sortedSeasonEvals = [...seasonEvals].sort(
        (a, b) => new Date(a.data).getTime() - new Date(b.data).getTime(),
      );
      ...
      const seasonData = {
        ...
        average:
          seasonEvals.length > 0
            ? seasonEvals.reduce((sum, e) => sum + e.nota, 0) /
              seasonEvals.length
            : 0,
    ...
    return (
      <div className="text-center space-y-4">
        ...
        <p className="text-muted-foreground">
          Não foi possível carregar a lista de atendentes. Verifique sua conexão
          e tente novamente.
        </p>
  ...
  if (!attendant) {
    return (
      <div className="text-center space-y-4">
        ...
        <p className="text-muted-foreground">
          O atendente com ID "{id}" não foi encontrado ou pode ter sido
          removido.
        </p>
        <div className="bg-muted p-4 rounded-lg text-sm">
          <p>
            <strong>ID procurado:</strong> {id}
          </p>
          <p>
            <strong>Total de atendentes:</strong> {attendants.data?.length || 0}
          </p>
        </div>
    ...
    <div className="space-y-6">
      <div className="flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-muted-foreground">
            Desempenho, conquistas e progressão
          </p>
        </div>
      </div>
              ...
              <div className="flex items-start justify-between">
                <div>
                  ...
                  <div className="flex items-center gap-2 mt-1">
                    ...
                    <Badge
                      variant={
                        attendant.status === "Ativo" ? "default" : "destructive"
                      }
                    >
                      {attendant.status}
                    </Badge>
                  ...
                  <div className="flex items-center gap-4 mt-2 text-sm text-muted-foreground">
                    ...
                    <span>
                      Desde{" "}
                      {format(new Date(attendant.dataAdmissao), "MMM/yyyy", {
                        locale: ptBR,
                      })}
                    </span>
                  </div>
                  ...
                  <div className="space-y-1 mt-2">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="font-semibold">
                        {Math.round(stats.totalXp)} XP Total
                      </span>
                    </div>
                    {stats.currentSeason && (
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-sm font-medium">
                          {Math.round(stats.currentSeasonXp)} XP (
              ...
              {unlockedAchievements
                ...
                .map((unlocked) => {
                  ...
                  const achievement = Array.isArray(achievements)
                    ? achievements.find(
                        (a) => a && a.id === unlocked.achievementId,
                      )
                    : null;
      ...
      <Tabs defaultValue="overview" className="space-y-4">
            ...
            <Card>
              ...
              <CardContent className="space-y-4">
                ...
                <div className="flex items-center justify-between">
                  ...
                  <Badge
                    variant={
                      stats.streaks.current > 0 ? "default" : "secondary"
                    }
                  >
                ...
                <div className="flex items-center justify-between">
                  ...
                  <Badge variant="outline">
                    {Math.round(stats.totalXp)} pontos
                  </Badge>
                </div>
              </CardContent>
            </Card>
            ...
            <div>
              <div className="flex items-center gap-2 mb-4">
                ...
                <Badge variant="secondary">
                  Últimas {Math.min(6, unlockedAchievements.length)}{" "}
                  desbloqueadas
                </Badge>
              </div>
        ...
        <TabsContent value="seasons" className="space-y-4">
          <div className="grid gap-4">
            {Object.entries(stats.seasonStats).map(
                    ...
                    <div className="flex items-start justify-between">
                      <div className="flex-1">
                        ...
                        <p className="text-sm text-muted-foreground mb-3">
                          ...
                          -{" "}
                          {format(new Date(seasonData.endDate), "dd/MM/yyyy", {
                            locale: ptBR,
                          })}
                        </p>
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                          ...
                          <div>
                            ...
                            <p className="font-semibold">
                              {seasonData.achievements.length}
                            </p>
                          </div>
                          <div>
                            <p className="text-muted-foreground">
                      ...
                      <div className="text-right">
                        <div className="flex items-center gap-1 mb-1">
                          ...
                          <span className="font-semibold text-lg">
                            {Math.round(seasonData.xp)} XP
                          </span>
                        </div>
                        {seasonData.isActive &&
                          seasonData.streaks.current > 0 && (
                            <Badge variant="outline" className="text-xs">
              ...
              <CardHeader>
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <CardDescription>
                      {format(new Date(seasonData.startDate), "dd/MM/yyyy", {
                        locale: ptBR,
                      })}{" "}
                      -{" "}
                      {format(new Date(seasonData.endDate), "dd/MM/yyyy", {
                  ...
                  <Badge
                    variant={seasonData.isActive ? "default" : "secondary"}
                  >
              ...
              <CardContent className="space-y-4">
                  ...
                  <div>
                    ...
                    <div className="grid gap-2 md:grid-cols-2">
                      {seasonData.achievements.map((achievement: any) => {
                        const config = achievements.find(
                ...
                {seasonData.evaluations > 0 && (
                  <div>
                    ...
                    <div className="space-y-2">
                      {Object.entries(seasonData.ratingDistribution).map(
                        ([rating, count]: [string, any]) => (
                          <div key={rating} className="flex items-center gap-3">
                            ...
                            <Progress
                              value={
                                seasonData.evaluations > 0
                                  ? (count / seasonData.evaluations) * 100
                                  : 0
                              }
                              className="flex-1 h-2"
                            />
          ...
          {stats.seasonHistory.length === 0 && (
            <Card>
              <CardContent className="p-8 text-center">
                ...
                <p className="text-muted-foreground">
                  Este atendente ainda não possui dados em temporadas
                  anteriores.
                </p>
              </CardContent>
            </Card>
        ...
        <TabsContent value="xp-events" className="space-y-4">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Todos os eventos que geraram pontos de experiência
              </CardDescription>
            </CardHeader>
            <CardContent>
              <Table>
                ...
                <TableBody>
                  {xpEvents
                    ...
                    .map((event, index) => {
                      const eventSeason = seasons?.find(
                        (s) => s.id === event.seasonId,
                      );
                      return (
                        <TableRow key={index}>
                          ...
                          <TableCell>
                            <Badge
                              variant={
                                event.points >= 0 ? "secondary" : "destructive"
                              }
