...
import { Crown, UserCircle, Shield, Calendar, Trophy, TrendingUp, Users, Loader2 } from "lucide-react";
...
export default function HistoricoTemporadasPage() {
    ...
    useEffect(() => {
        ...
        if (availableSeasons.length > 0 && !selectedSeasonId && xpEvents && Array.isArray(xpEvents)) {
            // Encontrar a temporada finalizada com mais eventos
            const seasonWithMostEvents = availableSeasons.find(season => {
                const seasonEvents = xpEvents.filter(e => e.seasonId === season.id);
    ...
    }, [isAuthenticated, loading, router, availableSeasons, selectedSeasonId, xpEvents]);
    ...
    const leaderboard = useMemo(() => {
        if (!selectedSeason || !xpEvents || !Array.isArray(xpEvents) || !attendants || !Array.isArray(attendants)) {
            return [];
        }
        ...
        try {
            ...
            const seasonXpEvents = xpEvents.filter(e => e.seasonId === selectedSeason.id);
            
            console.log(`Eventos XP para ${selectedSeason.name}:`, seasonXpEvents.length);
            ...
            if (seasonXpEvents.length === 0) {
                ...
                eventsToUse = xpEvents.filter(e => {
                    const eventDate = new Date(e.date);
                    return eventDate >= seasonStartDate && eventDate <= seasonEndDate;
                });
                
                console.log(`Fallback - eventos por data para ${selectedSeason.name}:`, eventsToUse.length);
            }

            // Agrupar XP por atendente
            const xpByAttendant = new Map<string, { totalXp: number; eventCount: number }>();
            
            eventsToUse.forEach(event => {
                const current = xpByAttendant.get(event.attendantId) || { totalXp: 0, eventCount: 0 };
            ...
            // Criar leaderboard
            const leaderboardData = attendants
                .map(attendant => {
    ...
    const seasonStats = useMemo(() => {
        if (!selectedSeason || !xpEvents || !Array.isArray(xpEvents)) return null;

        const seasonEvents = xpEvents.filter(e => e.seasonId === selectedSeason.id);
        ...
        const participantCount = new Set(seasonEvents.map(e => e.attendantId)).size;
                        ...
                        <CardContent className="p-4">
                            <div className="flex items-center gap-2">
                                ...
                                <div>
                                    ...
                                    <p className="text-2xl font-bold">{seasonStats.totalXp.toLocaleString()}</p>
                                </div>
                            </div>
                        </CardContent>
                    </Card>
                        ...
                        <CardContent className="p-4">
                            <div className="flex items-center gap-2">
                                ...
                                <div>
                                    ...
                                    <p className="text-2xl font-bold">{seasonStats.xpMultiplier}x</p>
                                </div>
                            </div>
                        </CardContent>
                    </Card>
                            ...
                            <TableBody>
                                {leaderboard.length > 0 ? leaderboard.map((att, index) => (
                                    <TableRow key={att.id} className={index < 3 ? 'bg-amber-50 dark:bg-amber-950/50' : ''}>
                                        ...
                                        <TableCell className="text-right font-bold text-lg">{att.score.toLocaleString()}</TableCell>
                                    </TableRow>
                                )) : (
                                    <TableRow>
                                            ...
                                            <div className="flex flex-col items-center gap-2 text-muted-foreground">
                                                ...
                                                <div>
                                                    <p className="font-medium">
                                                        {selectedSeasonId ? "Nenhum dado encontrado" : "Selecione uma temporada"}
                                                    </p>
                                                    <p className="text-sm">
                                                        {selectedSeasonId 
            ...
            {process.env.NODE_ENV === 'development' && selectedSeason && (
                <Card className="border-dashed">
                    ...
                    <CardContent className="text-xs space-y-1">
                        ...
                        <p>Total XP Events: {xpEvents?.length || 0}</p>
                        <p>Season XP Events: {xpEvents?.filter(e => e.seasonId === selectedSeason.id).length || 0}</p>
                        <p>Attendants: {attendants?.length || 0}</p>
