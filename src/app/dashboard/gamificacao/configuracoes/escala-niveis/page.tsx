...
export default function EscalaNiveisPage() {
  ...
  const { attendants, seasonXpEvents, activeSeason, nextSeason, isAnyLoading } =
    useApi();
  ...
  const systemStats = useMemo(() => {
    ...
    leaderboard.forEach((att) => {
      levelDistribution.set(
        att.level,
        (levelDistribution.get(att.level) || 0) + 1,
      );
    });

    const topLevel = leaderboard[0];
    const progressToMax = topLevel
      ? (topLevel.score / getXpForLevel(MAX_LEVEL)) * 100
      : 0;
    ...
    <div className="space-y-8">
      <div>
        ...
        <p className="text-muted-foreground">
          Gerencie e acompanhe o sistema de níveis, escalas de XP e progresso da
          equipe.
        </p>
      </div>
      ...
      <SeasonStatus
        ...
        nextSeason={nextSeason || undefined}
      />
          ...
          <CardContent className="p-4">
            <div className="flex items-center gap-2">
              ...
              <div>
                ...
                <p className="text-2xl font-bold">
                  {systemStats.averageXp.toLocaleString()}
                </p>
              </div>
            </div>
          </CardContent>
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
                      ...
                      >
                        ...
                        <div className="flex items-center gap-2">
                          <Badge variant="secondary">
                            {count} atendente{count !== 1 ? "s" : ""}
                          </Badge>
                          <div className="w-20 bg-muted rounded-full h-2">
                            <div
                              className="bg-blue-500 h-2 rounded-full"
                              style={{
                                width: `${(count / systemStats.totalAttendants) * 100}%`,
                              }}
                            />
                          </div>
                        </div>
                  ...
                  <div className="space-y-4">
                    <div className="flex items-center gap-4">
                      <Avatar className="h-16 w-16">
                        <AvatarImage
                          src={topAttendant.avatarUrl || undefined}
                          alt={topAttendant.name}
                        />
                        <AvatarFallback>
                          <UserCircle />
                        </AvatarFallback>
                    ...
                    <div className="space-y-2">
                      <div className="flex justify-between text-sm">
                        ...
                        <span className="font-bold">
                          {Math.round(topAttendant.score).toLocaleString()}
                        </span>
                      </div>
                      <div className="flex justify-between text-sm">
                        <span>
                          Progresso para Nível {topAttendant.level + 1}
                        </span>
                        <span className="font-bold">
                          {Math.round(topAttendant.progress)}%
                        </span>
                      </div>
        ...
        <TabsContent value="rewards">
          <Card>
            <CardHeader>
              <CardTitle>
                Trilha de Recompensas (Nível 1 a {MAX_LEVEL})
              </CardTitle>
              <CardDescription>
