...
const formSchema = z.object({
  globalXpMultiplier: z.coerce
    .number()
    .min(0, "O multiplicador não pode ser negativo.")
    .default(1),
  seasons: z.array(
    z.object({
      id: z.string(),
  ...
  async function onSubmit(values: z.infer<typeof formSchema>) {
    try {
      await updateGamificationConfig({
        globalXpMultiplier: values.globalXpMultiplier,
      });
        ...
        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Este fator se aplica a todas as fontes de XP (avaliações e
                troféus), em conjunto com os multiplicadores de temporada.
              </CardDescription>
            </CardHeader>
            <CardContent>
              <FormField
                control={form.control}
                name="globalXpMultiplier"
                render={({ field }) => (
                  <FormItem>
            ...
            <CardHeader>
              <div className="flex justify-between items-start">
                <div>
                  ...
                  <CardDescription>
                    Defina um multiplicador específico para cada temporada. Ele
                    será combinado com o global.
                  </CardDescription>
                </div>
                <Button variant="outline" asChild>
                  <Link href="/dashboard/gamificacao/configuracoes/sessoes">
                    <Edit className="mr-2 h-4 w-4" /> Gerenciar Sessões
                  </Link>
                </Button>
              </div>
            </CardHeader>
                    ...
                    return (
                      <TableRow key={field.id}>
                        ...
                        <TableCell>
                          <Badge
                            variant={
                              seasonInfo.active ? "secondary" : "destructive"
                            }
                          >
                        ...
                        <TableCell>
                          {format(new Date(seasonInfo.startDate), "dd/MM/yy")} -{" "}
                          {format(new Date(seasonInfo.endDate), "dd/MM/yy")}
                        </TableCell>
                        <TableCell>
                          <FormField
                            control={form.control}
