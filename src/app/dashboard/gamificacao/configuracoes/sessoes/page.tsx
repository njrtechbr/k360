...
import { MoreHorizontal, Pencil, PlusCircle, Trash2, CalendarIcon, Percent, AlertTriangle, CheckCircle, Clock, Play, Pause } from "lucide-react";
...
const formSchema = z.object({
  ...
  active: z.boolean(),
  xpMultiplier: z.coerce.number().min(0.1, "O multiplicador deve ser pelo menos 0.1").max(10, "O multiplicador não pode ser maior que 10").default(1),
}).refine((data) => {
  // Validar que o período tem pelo menos 1 dia
  const diffTime = data.dateRange.to.getTime() - data.dateRange.from.getTime();
...
export default function ConfigurarSessoesPage() {
    ...
    const [conflictingSeason, setConflictingSeason] = useState<GamificationSeason | null>(null);
    ...
    const canActivateSeason = (season: GamificationSeason) => {
        ...
        if (seasonStats.activeSeason && seasonStats.activeSeason.id !== season.id) return false;
    ...
    async function onSubmit(values: z.infer<typeof formSchema>) {
        try {
            ...
            if (values.active) {
                if (seasonStats.activeSeason && seasonStats.activeSeason.id !== selectedSeason?.id) {
                    toast({
                        variant: "destructive",
            ...
            if (selectedSeason) {
                await updateGamificationSeason(selectedSeason.id, seasonData);
                toast({
                    title: "Temporada atualizada!",
                    description: `A temporada "${seasonData.name}" foi atualizada com sucesso.`
                });
            } else {
                await addGamificationSeason(seasonData);
                toast({
    ...
    const handleToggleActive = async (season: GamificationSeason, active: boolean) => {
        try {
            await updateGamificationSeason(season.id, { active });
            toast({
    ...
    const handleForceActivate = async () => {
        ...
        try {
            ...
            await updateGamificationSeason(conflictingSeason.id, { active: false });
            // Ativar nova temporada
            await updateGamificationSeason(selectedSeason.id, { active: true });
    ...
    async function onDeleteConfirm() {
        ...
        try {
            await deleteGamificationSeason(selectedSeason.id);
            toast({
        ...
        <div className="space-y-8">
            <div className="flex items-center justify-between">
                <div>
                    ...
                    <p className="text-muted-foreground">
                        Gerencie os períodos de competição. Apenas uma temporada pode estar ativa por vez.
                    </p>
                </div>
            ...
            {seasonStats.activeSeason && (
                <Alert>
                    ...
                    <AlertDescription>
                        ...
                        ({format(new Date(seasonStats.activeSeason.startDate), 'dd/MM/yyyy')} - {format(new Date(seasonStats.activeSeason.endDate), 'dd/MM/yyyy')})
                        {seasonStats.activeSeason.xpMultiplier !== 1 && (
                            <span className="ml-2">
                                <Badge variant="outline">{seasonStats.activeSeason.xpMultiplier}x XP</Badge>
                            </span>
                        )}
                    </AlertDescription>
                </Alert>
            ...
            <Card>
                <CardHeader>
                    ...
                    <CardDescription>
                        Gerencie suas temporadas. Lembre-se: apenas uma pode estar ativa por vez e os períodos não podem se sobrepor.
                    </CardDescription>
                </CardHeader>
                <CardContent>
                    {sortedSeasons.length === 0 ? (
                        <div className="text-center py-8">
                            ...
                            <h3 className="mt-2 text-sm font-semibold text-muted-foreground">Nenhuma temporada</h3>
                        ...
                        <Table>
                            ...
                            <TableBody>
                                {sortedSeasons.map((season) => {
                                    ...
                                    return (
                                        <TableRow key={season.id} className={season.active ? "bg-green-50" : ""}>
                                            <TableCell>
                                                <div className="font-medium">{season.name}</div>
                                                <div className="text-sm text-muted-foreground">
                                                    {Math.ceil((new Date(season.endDate).getTime() - new Date(season.startDate).getTime()) / (1000 * 60 * 60 * 24))} dias
                                                </div>
                                            </TableCell>
                                            <TableCell>
                                                <div className="text-sm">
                                                    <div>{format(new Date(season.startDate), 'dd/MM/yyyy')}</div>
                                            ...
                                            <TableCell className="text-right">
                                                <div className="flex items-center justify-end gap-2">
                                                    ...
                                                    {(canActivate || season.active) && (
                                                        <Button
                                                            variant={season.active ? "destructive" : "default"}
                                                            size="sm"
                        ...
                        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
                            ...
                            <FormField control={form.control} name="dateRange" render={({ field }) => (
                                <FormItem className="flex flex-col">
                                    ...
                                    <FormDescription>
                                        Selecione as datas de início e fim. Os períodos não podem se sobrepor.
                                    </FormDescription>
                                    <FormMessage />
                                </FormItem>
                            )} />
                            ...
                            <FormField control={form.control} name="xpMultiplier" render={({ field }) => (
                                <FormItem>
                                    ...
                                    <FormDescription>
                                        Multiplicador aplicado ao XP durante esta temporada (0.1 - 10.0).
                                    </FormDescription>
                                    <FormMessage />
                                </FormItem>
                            )} />
                            ...
                            <FormField control={form.control} name="active" render={({ field }) => (
                                <FormItem className="flex flex-row items-center justify-between rounded-lg border p-4">
                                    <div className="space-y-0.5">
                                        <FormLabel className="text-base">Ativar Temporada</FormLabel>
                                        <FormDescription>
                                            {field.value 
                                                ? "Esta temporada será ativada imediatamente." 
                                                : "Esta temporada ficará inativa até ser ativada manualmente."
                                            }
                                            {seasonStats.activeSeason && !selectedSeason && field.value && (
                                                <div className="mt-2 text-amber-600">
                                                    ⚠️ Já existe uma temporada ativa. Ela será desativada automaticamente.
                                                </div>
                                            )}
                                        </FormDescription>
                                    </div>
                            ...
                            <DialogFooter>
                                ...
                                <Button type="submit" disabled={form.formState.isSubmitting}>
                                    {form.formState.isSubmitting ? (
                                        ...
                                        selectedSeason ? "Atualizar" : "Criar"
                                    )}
                                </Button>
                            </DialogFooter>
                        </form>
                ...
                <AlertDialogContent>
                    <AlertDialogHeader>
                        ...
                        <AlertDialogDescription>
                            Tem certeza que deseja excluir a temporada <strong>"{selectedSeason?.name}"</strong>?
                            <br /><br />
                            Esta ação não pode ser desfeita. A temporada será removida permanentemente, 
                            mas os dados de XP dos atendentes não serão afetados.
                        </AlertDialogDescription>
                    </AlertDialogHeader>
                    <AlertDialogFooter>
            ...
            <AlertDialog open={isActivateDialogOpen} onOpenChange={setIsActivateDialogOpen}>
                <AlertDialogContent>
                    <AlertDialogHeader>
                        ...
                        <AlertDialogDescription>
                            Já existe uma temporada ativa: <strong>"{conflictingSeason?.name}"</strong>.
                            <br /><br />
                            Para ativar <strong>"{selectedSeason?.name}"</strong>, a temporada atual será 
                            automaticamente desativada. Deseja continuar?
                        </AlertDialogDescription>
                    </AlertDialogHeader>
                    <AlertDialogFooter>
