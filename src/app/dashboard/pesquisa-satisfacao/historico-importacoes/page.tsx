...
export default function HistoricoImportacoesPage() {
  ...
  const [selectedImport, setSelectedImport] = useState<EvaluationImport | null>(
    null,
  );
  ...
  const evaluationsForSelectedImport = useMemo(() => {
    if (!selectedImport || !evaluations.data) return [];
    const evaluationIds = new Set(selectedImport.evaluationIds);
    return evaluations.data.filter((ev) => evaluationIds.has(ev.id));
  }, [selectedImport, evaluations.data]);
  ...
  const handleRevertConfirm = async () => {
    ...
    await deleteEvaluationImport.mutate(selectedImport.id);
  ...
  const handleDeleteSelectedConfirm = async () => {
    ...
    await deleteEvaluations.mutate({
      evaluationIds: Array.isArray(evaluationsToDelete)
        ? evaluationsToDelete.map((e) => e.id)
        : [],
      title: "Excluindo Avaliações Selecionadas",
    });
    ...
    if (selectedImport) {
      const updatedImport = {
        ...selectedImport,
        evaluationIds: selectedImport.evaluationIds.filter(
          (id) => !selectedEvaluationIds.includes(id),
        ),
      };
  ...
  const sortedImports = evaluationImports.data
    ? [...evaluationImports.data].sort(
        (a, b) =>
    ...
    <>
      <div className="space-y-8">
        <h1 className="text-3xl font-bold">
          Gerenciar Importações de Avaliações
        </h1>
        <Card>
          <CardHeader>
            <CardTitle>Lotes Importados</CardTitle>
            <CardDescription>
              Gerencie os lotes de avaliações importados. Você pode ver os
              detalhes de cada lote, reverter uma importação inteira ou excluir
                    ...
                    <TableRow key={importRecord.id}>
                      ...
                      <TableCell>
                        <div className="flex items-center gap-2">
                          ...
                          {userMap[importRecord.importedBy] ||
                      ...
                      <TableCell className="text-right space-x-2">
                        <Button
                          variant="outline"
                          size="sm"
                          onClick={() => handleDetailsClick(importRecord)}
                        >
                        ...
                        <Button
                          variant="destructive"
                          size="sm"
                          onClick={() => handleRevertClick(importRecord)}
                        >
      ...
      <Dialog open={isDetailsDialogOpen} onOpenChange={setIsDetailsDialogOpen}>
        <DialogContent className="max-w-4xl">
          <DialogHeader>
            ...
            <DialogDescription>
              Arquivo:{" "}
              <span className="font-semibold">{selectedImport?.fileName}</span>.
              Visualize e gerencie as avaliações deste lote.
            </DialogDescription>
          </DialogHeader>
                    ...
                    <Checkbox
                      checked={
                        ...
                        selectedEvaluationIds.length ===
                          evaluationsForSelectedImport.filter(
                            (e) => e.importId !== "native",
                          ).length
                      }
                  ...
                  <TableRow key={ev.id}>
                    ...
                    <TableCell>
                      <Badge variant="outline">
                        {attendantMap[ev.attendantId] || "Desconhecido"}
                      </Badge>
                    </TableCell>
                    <TableCell>
                      <RatingStars rating={ev.nota} />
                    </TableCell>
          ...
          <DialogFooter className="justify-between sm:justify-between">
            ...
            >
              ...
              {selectedEvaluationIds.length} Selecionadas
            </Button>
            <Button
              variant="outline"
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle className="flex items-center gap-2">
              ...
              Reverter Importação Completa?
            </AlertDialogTitle>
            <AlertDialogDescription>
              Você tem certeza que deseja reverter este lote de importação?
              Todas as <strong>{selectedImport?.evaluationIds.length}</strong>{" "}
              avaliações associadas ao arquivo{" "}
              <strong>{selectedImport?.fileName}</strong> serão permanentemente
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              ...
              <strong>{selectedEvaluationIds.length}</strong> avaliações
              selecionadas deste lote? Esta ação é irreversível.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel>Cancelar</AlertDialogCancel>
