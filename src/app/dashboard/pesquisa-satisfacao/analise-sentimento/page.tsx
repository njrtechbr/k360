...
import SentimentFilters, {
  SentimentFilterOptions,
  AttendantOption,
} from "@/components/survey/SentimentFilters";
import {
  safeReduceArray,
  safeMapArray,
  isValidAttendant,
...
export default function AnaliseSentimentoPage() {
  ...
  const filteredAnalyses = useMemo(() => {
    ...
    if (filters.searchTerm) {
      ...
      filtered = filtered.filter(
        (analysis) =>
          ...
          (attendantMap[analysis.evaluation.attendantId] || "")
            .toLowerCase()
    ...
    if (filters.confidenceRange) {
      ...
      filtered = filtered.filter(
        (analysis) => analysis.confidence >= min && analysis.confidence <= max,
      );
    }
    ...
    filtered.sort((a, b) => {
      ...
      switch (sortBy) {
        ...
        default:
          comparison =
            new Date(a.evaluation.data).getTime() -
            new Date(b.evaluation.data).getTime();
          break;
      }
  ...
  const attendantOptions: AttendantOption[] = useMemo(() => {
    ...
    return safeMapArray(
      attendants,
      (attendant) => {
        const totalAnalyses = allAnalysisWithDetails.filter(
          (a) => a.evaluation.attendantId === attendant.id,
        ).length;
  ...
  if (!Array.isArray(attendants)) {
    console.warn(
      "AnaliseSentimentoPage: attendants não é um array válido",
      attendants,
    );
  }
  ...
  if (!Array.isArray(evaluations)) {
    console.warn(
      "AnaliseSentimentoPage: evaluations não é um array válido",
      evaluations,
    );
  }
  ...
  if (!Array.isArray(aiAnalysisResults)) {
    console.warn(
      "AnaliseSentimentoPage: aiAnalysisResults não é um array válido",
      aiAnalysisResults,
    );
  }
  ...
  const sortedAnalysis = [...filteredAnalyses].sort(
    (a, b) =>
      new Date(b.evaluation.data).getTime() -
      new Date(a.evaluation.data).getTime(),
  );
  ...
  const lastEvaluationAnalyzed = lastResult
    ? evaluationMap[lastResult.evaluationId]
    : null;
    ...
    <>
      <div className="space-y-8">
        <div className="flex flex-col sm:flex-row justify-between items-start gap-4">
          <div>
            ...
            <p className="text-muted-foreground">
              Analise os comentários das avaliações para extrair insights.
            </p>
          </div>
        </div>
        ...
        <AnalysisProgress
          ...
          analyzedCount={
            Array.isArray(aiAnalysisResults) ? aiAnalysisResults.length : 0
          }
          pendingCount={
            Array.isArray(evaluations) && Array.isArray(aiAnalysisResults)
              ? evaluations.length - aiAnalysisResults.length
              : 0
          }
          sentimentDistribution={{
            positive: Array.isArray(filteredAnalyses)
              ? filteredAnalyses.filter((a) => a.sentiment === "Positivo")
                  .length
              : 0,
            negative: Array.isArray(filteredAnalyses)
              ? filteredAnalyses.filter((a) => a.sentiment === "Negativo")
                  .length
              : 0,
            neutral: Array.isArray(filteredAnalyses)
              ? filteredAnalyses.filter((a) => a.sentiment === "Neutro").length
              : 0,
          }}
          recentAnalyses={
            Array.isArray(sortedAnalysis) ? sortedAnalysis.slice(0, 3) : []
          }
                  ...
                  <CardHeader>
                    <div className="flex items-center justify-between">
                      <div>
                        ...
                        <CardDescription>
                          {filteredAnalyses.length} de{" "}
                          {allAnalysisWithDetails.length} análises encontradas
                        </CardDescription>
                      </div>
                    </div>
                  </CardHeader>
                          ...
                          <TableRow key={item.evaluationId}>
                            <TableCell className="font-medium">
                              <Badge variant="outline">
                                {attendantMap[item.evaluation.attendantId] ||
                                  "Desconhecido"}
                              </Badge>
                            </TableCell>
                            <TableCell>
      ...
      <Dialog open={isProgressModalOpen} onOpenChange={setIsProgressModalOpen}>
        ...
        >
          <DialogHeader>
            ...
            <DialogDescription>
              O processo está rodando em tempo real. Por favor, aguarde a
              conclusão.
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-4">
                ...
                <Card className="bg-muted/50 animate-in fade-in">
                  <CardHeader>
                    <CardTitle className="text-base flex items-center gap-2">
                      <Hourglass className="animate-spin text-amber-500" />{" "}
                      Analisando agora...
                    </CardTitle>
                  </CardHeader>
                  <CardContent className="space-y-3">
                    <div className="flex items-center gap-2">
                      ...
                      <Badge variant="outline">
                        {attendantMap[
                          analysisProgress?.evaluation?.attendantId
                        ] || "Desconhecido"}
                      </Badge>
                    </div>
            ...
            {analysisProgress?.status === "waiting" &&
              lastResult &&
              lastEvaluationAnalyzed && (
                <Card className="bg-muted/50 animate-in fade-in">
                  <CardHeader>
                  ...
                  <CardContent className="space-y-3">
                    <div className="flex items-center gap-2">
                      <span className="font-semibold">Atendente:</span>
                      <Badge variant="outline">
                        {attendantMap[lastEvaluationAnalyzed.attendantId] ||
                          "Desconhecido"}
                      </Badge>
                    </div>
                    <div className="flex items-center gap-2">
                    ...
                    <div className="pt-2 text-center text-sm text-muted-foreground">
                      Próxima análise em {analysisProgress?.countdown || 0}{" "}
                      segundos...
                    </div>
                  </CardContent>
                </Card>
