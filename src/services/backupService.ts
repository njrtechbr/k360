...
export class BackupService {
  private static readonly REGISTRY_PATH = path.join(
    process.cwd(),
    "backups",
    "registry.json",
  );
  private static readonly DEFAULT_BACKUP_DIR =
    process.env.BACKUP_DIRECTORY || "./backups/files";
  private static readonly PGDUMP_PATH = process.env.PGDUMP_PATH || "pg_dump";
  private static readonly PGDUMP_TIMEOUT =
    parseInt(process.env.PGDUMP_TIMEOUT || "3600") * 1000;
  private static readonly MAX_SIZE_GB = parseInt(
  ...
  static async createBackup(
    options: BackupOptions & { createdBy?: string; backupId?: string } = {},
  ): Promise<BackupResult & { id?: string }> {
    ...
    return await BackupErrorHandler.executeWithRetry(
      async () => {
        ...
        try {
          ...
          const dumpResult = await this.executePgDumpWithRetry(
            filepath,
            options,
            (progress) => {
              if (updateProgressCallback) {
          ...
          if (!dumpResult.success) {
            if (updateProgressCallback) {
              updateProgressCallback(
                backupId,
                0,
                dumpResult.error || "Falha na execução do pg_dump",
                "failed",
              );
            }
          ...
          const validationResult = await BackupValidator.validateBackup(
            filepath,
            checksum,
          );
   ...
   */
  private static async validateBackupOptionsWithRetry(
    ...
    return await BackupErrorHandler.executeWithRetry(
      async () => {
        if (
          options.filename &&
          !/^[a-zA-Z0-9_-]+\.sql(\.gz)?$/.test(options.filename)
        ) {
          throw new ValidationError(
            "Nome do arquivo inválido. Use apenas letras, números, _ e -",
        ...
        if (
          ...
          throw new ValidationError(
            "Diretório deve ser um caminho absoluto ou relativo válido",
   ...
   */
  private static async ensureDirectoryExistsWithRetry(
    directory: string,
  ): Promise<void> {
    return await BackupErrorHandler.executeWithRetry(
      async () => {
            ...
            try {
              ...
              if (mkdirError?.code === "EACCES") {
                throw new PermissionError(
                  `Sem permissão para criar diretório: ${directory}`,
                  { directory, error: mkdirError },
                );
              }
              throw new FileSystemError(
                `Falha ao criar diretório: ${directory}`,
                { directory, error: mkdirError },
              );
            }
          } else if (error?.code === "EACCES") {
            throw new PermissionError(
              `Sem permissão para acessar diretório: ${directory}`,
              { directory, error },
            );
          } else {
            throw new FileSystemError(
              `Erro ao acessar diretório: ${directory}`,
              { directory, error },
            );
          }
        }
   ...
   */
  private static async checkDiskSpaceWithRetry(
    directory: string,
  ): Promise<void> {
    return await BackupErrorHandler.executeWithRetry(
      async () => {
  ...
  private static async executePgDumpWithRetry(
    ...
    return await BackupErrorHandler.executeWithRetry(
      async () => {
        const result = await this.executePgDump(
          filepath,
          options,
          progressCallback,
        );
        if (!result.success) {
          ...
          if (
            ...
            throw new DatabaseConnectionError(result.error, {
              filepath,
              options,
            });
          } else if (
            ...
            throw new BackupCreationError(
              result.error || "Falha desconhecida no pg_dump",
              { filepath, options },
            );
          }
        }
  ...
  private static buildPgDumpArgs(options: BackupOptions): string[] {
    ...
    if (!databaseUrl) {
      throw new DatabaseConnectionError("DATABASE_URL não configurada", {
        options,
      });
    }
    ...
    try {
      ...
      throw new DatabaseConnectionError("URL do banco de dados inválida", {
        databaseUrl,
        error,
        options,
      });
   ...
   */
  private static async compressBackupWithRetry(
    filepath: string,
  ): Promise<string> {
    return await BackupErrorHandler.executeWithRetry(
      async () => {
    ...
    return new Promise(async (resolve, reject) => {
      try {
        ...
        archive.on("error", (error) => {
          reject(
            new CompressionError(`Falha na compressão: ${error.message}`, {
              filepath,
              error,
            }),
          );
        ...
        reject(
          new CompressionError(
            `Erro ao inicializar compressão: ${error instanceof Error ? error.message : "Erro desconhecido"}`,
            { filepath, error },
          ),
        );
      }
   ...
   */
  private static async getDatabaseInfoWithRetry(): Promise<{
    version: string;
   ...
   */
  private static async getDatabaseInfo(): Promise<{
    version: string;
    schemaVersion: string;
  }> {
    try {
      ...
      throw new DatabaseConnectionError(
        `Falha ao obter informações do banco: ${error instanceof Error ? error.message : "Erro desconhecido"}`,
        { error },
      );
    }
  }
   ...
   */
  private static async saveBackupMetadataWithRetry(
  ...
  private static async saveBackupMetadata(
    ...
    try {
      ...
      throw new RegistryError("Falha ao salvar metadados do backup", {
        metadata,
        error,
      });
    }
   ...
   */
  static async getBackupValidationDetails(
    filepath: string,
    expectedChecksum?: string,
  ) {
    ...
    try {
      ...
      if (backupsToValidate.length === 0) {
        throw new BackupError(
          "Nenhum backup válido encontrado para validação",
          "NO_BACKUPS_FOUND",
        );
      }
  ...
  static async runIntegrityCheck() {
    try {
      ...
      console.log(
        `Iniciando verificação de integridade de ${registry.backups.length} backup(s)...`,
      );

      const filepaths = registry.backups.map((b) => b.filepath);
      const expectedChecksums = registry.backups.reduce(
        (acc, backup) => {
      ...
      const validBackups = Object.values(results).filter(
        (r) => r.isValid,
      ).length;
      ...
      for (const backup of registry.backups) {
        ...
        if (result && !result.isValid) {
          console.warn(
            `Backup ${backup.filename} falhou na verificação de integridade:`,
            result.errors,
          );
        }
      }
      ...
      return {
        ...
        summary: {
          ...
          successRate:
            totalBackups > 0 ? (validBackups / totalBackups) * 100 : 0,
        },
      };
    } catch (error) {
  ...
  static async detectCorruptedBackups() {
    try {
      ...
      for (const backup of registry.backups) {
        try {
          const isCorrupted = await BackupValidator.detectFileCorruption(
            backup.filepath,
          );
          if (isCorrupted) {
      ...
      throw new BackupError(
        "Falha ao detectar backups corrompidos",
