...
export class BackupErrorHandler {
  ...
  static async executeWithRetry<T>(
    ...
    for (let attempt = 1; attempt <= config.maxAttempts; attempt++) {
      try {
        ...
        if (
          !config.retryableErrors.includes(backupError.errorType) ||
        ...
        console.warn(
          `Tentativa ${attempt}/${config.maxAttempts} falhou para ${operationName}. Tentando novamente em ${delay}ms...`,
        );
  ...
  private static async tryFallbackStrategies(
    ...
    for (const strategy of applicableStrategies) {
      try {
        ...
        console.warn(
          `Estratégia de fallback ${strategy.strategy} falhou:`,
          fallbackError,
        );
        await this.logError(
  ...
  static async logErrorResolution(
    ...
    try {
      ...
      console.info(
        `Erro ${error.id} resolvido usando estratégia: ${strategy} (tentativa ${finalAttempt})`,
      );
    } catch (logError) {
  ...
  static async generateErrorReport(days: number = 7): Promise<string> {
    ...
    lines.push(
      `Taxa de resolução: ${stats.totalErrors > 0 ? ((stats.resolvedErrors / stats.totalErrors) * 100).toFixed(1) : 0}%\n`,
    );
    ...
    if (recentErrors.length > 0) {
      ...
      for (const error of recentErrors.slice(0, 5)) {
        lines.push(
          `  [${error.timestamp.toISOString()}] ${error.errorType}: ${error.message}`,
        );
        if (!error.resolved) {
