...
import { RegistryError, FileSystemError, PermissionError } from './backupErrorHandler';
...
export class BackupStorage {
  ...
  static async initialize(): Promise<void> {
    try {
      ...
      throw new FileSystemError(`Falha ao inicializar BackupStorage: ${error instanceof Error ? error.message : 'Erro desconhecido'}`, { error });
    }
  }
  ...
  private static async createEmptyRegistry(): Promise<void> {
    ...
    await fs.writeFile(this.registryPath, JSON.stringify(emptyRegistry, null, 2), 'utf8');
  }
  ...
  private static async readRegistry(): Promise<BackupRegistry> {
    try {
      ...
      throw new RegistryError(`Falha ao ler registry: ${error instanceof Error ? error.message : 'Erro desconhecido'}`, { error });
    }
  }
  ...
  private static async writeRegistry(registry: BackupRegistry): Promise<void> {
    try {
      ...
      throw new RegistryError(`Falha ao escrever registry: ${error instanceof Error ? error.message : 'Erro desconhecido'}`, { error });
    }
  }
  ...
  static async addBackup(metadata: BackupMetadata): Promise<void> {
    ...
    const existingIndex = registry.backups.findIndex(b => b.id === metadata.id);
    if (existingIndex >= 0) {
      throw new RegistryError(`Backup com ID ${metadata.id} já existe`, { metadata });
    }
   ...
   */
  static async updateBackup(backupId: string, updates: Partial<BackupMetadata>): Promise<void> {
    await this.initialize();
    ...
    if (backupIndex === -1) {
      throw new RegistryError(`Backup com ID ${backupId} não encontrado`, { backupId });
    }
  ...
  static async searchBackups(searchTerm: string): Promise<BackupMetadata[]> {
    ...
    return registry.backups.filter(backup => 
 ...
 * Limpa backups antigos baseado nas configurações de retenção
  ...
  static async cleanupOldBackups(): Promise<{
    ...
    const backupsToRemove = registry.backups.filter(backup => 
    ...
    for (const backup of backupsToRemove) {
      try {
        ...
        errors.push(`Falha ao remover backup ${backup.id}: ${error instanceof Error ? error.message : 'Erro desconhecido'}`);
      }
    }
  ...
  static async cleanupExcessBackups(): Promise<{
    ...
    const backupsToRemove = successfulBackups.slice(registry.settings.maxBackups);
  ...
  static async cleanupFailedBackups(): Promise<{
    ...
    for (const backup of failedBackups) {
      try {
        ...
        errors.push(`Falha ao remover backup falhado ${backup.id}: ${error instanceof Error ? error.message : 'Erro desconhecido'}`);
      }
    }
  ...
  static async simulateCleanupOldBackups(daysToKeep?: number): Promise<{
    ...
    const wouldFreeSpace = backupsToRemove.reduce((sum, backup) => sum + backup.size, 0);
  ...
  static async performFullCleanup(): Promise<{
    ...
    return {
      totalRemoved: oldBackupsResult.removed + excessBackupsResult.removed + failedBackupsResult.removed,
      totalFreedSpace: oldBackupsResult.freedSpace + excessBackupsResult.freedSpace + failedBackupsResult.freedSpace,
      details: {
   ...
   */
  static async updateSettings(newSettings: Partial<BackupRegistry['settings']>): Promise<void> {
    await this.initialize();
  ...
  static async validateRegistry(): Promise<{
    ...
    try {
      ...
      return {
        isValid: false,
        issues: [`Erro ao validar registry: ${error instanceof Error ? error.message : 'Erro desconhecido'}`],
        fixedIssues: []
      };
    }
  }
