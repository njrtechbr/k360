...
export class AnalyticsService {
   ...
   */
  static calculateRatingDistribution(
  ...
  static calculateMonthlyTrends(
    ...
    for (let i = months - 1; i >= 0; i--) {
      ...
      const averageRating =
        totalEvaluations > 0
          ? monthEvaluations.reduce(
              (sum, evaluation) => sum + evaluation.nota,
              0,
            ) / totalEvaluations
          : 0;

      const satisfiedCount = monthEvaluations.filter(
        (evaluation) => evaluation.nota >= 4,
      ).length;
      const satisfactionRate =
        totalEvaluations > 0 ? (satisfiedCount / totalEvaluations) * 100 : 0;
  ...
  static calculateAnalyticsData(
    ...
    const averageRating =
      totalEvaluations > 0
        ? evaluations.reduce((sum, evaluation) => sum + evaluation.nota, 0) /
          totalEvaluations
        : 0;
    ...
    evaluations.forEach((evaluation) => {
      const current = attendantStats.get(evaluation.attendantId) || {
        total: 0,
        sum: 0,
      };
  ...
  static calculatePeriodComparison(
    ...
    const previousPeriodStart = new Date(
      now.getTime() - currentPeriodDays * 2 * 24 * 60 * 60 * 1000,
    );

    const currentPeriodEvals = evaluations.filter(
      (evaluation) => new Date(evaluation.data) >= currentPeriodStart,
    );
  ...
  static calculateSectorStats(
    ...
    const sectorStats = new Map<
      string,
      { total: number; sum: number; evaluations: Evaluation[] }
    >();
    ...
    return Array.from(sectorStats.entries())
      .map(([sector, stats]) => {
        const satisfiedCount = stats.evaluations.filter(
          (evaluation) => evaluation.nota >= 4,
        ).length;
   ...
   */
  static generateInsights(evaluations: Evaluation[], attendants: Attendant[]) {
    ...
    if (comparison.changes.averageRating > 0.2) {
      insights.push(
        "📈 Melhoria significativa na satisfação dos clientes no último mês",
      );
    } else if (comparison.changes.averageRating < -0.2) {
      insights.push(
        "📉 Queda na satisfação dos clientes no último mês - atenção necessária",
      );
    }
    ...
    if (
      bestSector &&
      worstSector &&
      bestSector.averageRating - worstSector.averageRating > 1
    ) {
      insights.push(
        `🏆 Setor "${bestSector.sector}" se destaca com nota média de ${bestSector.averageRating.toFixed(1)}`,
      );
      insights.push(
        `⚠️ Setor "${worstSector.sector}" precisa de atenção com nota média de ${worstSector.averageRating.toFixed(1)}`,
      );
    }

    // Análise de comentários
    const withComments = evaluations.filter(
      (evaluation) =>
    ...
    if (commentRate > 70) {
      insights.push(
        "💬 Alto engajamento dos clientes com comentários detalhados",
      );
    } else if (commentRate < 30) {
      insights.push(
        "💭 Baixo engajamento com comentários - considere incentivar feedback",
      );
    }
