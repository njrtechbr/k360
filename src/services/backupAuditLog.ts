...
export class BackupAuditLogger {
  ...
  static async logOperation(
    entry: Omit<BackupAuditEntry, "id" | "timestamp">,
  ): Promise<void> {
    try {
      ...
      console.log(
        `[BACKUP AUDIT] ${entry.operation} by ${entry.userEmail} - ${entry.success ? "SUCCESS" : "FAILED"}`,
      );
    } catch (error) {
  ...
  static async getAuditEntries(
    filter: BackupAuditFilter = {},
  ): Promise<BackupAuditEntry[]> {
    try {
      const exists = await fs
        .access(this.auditFilePath)
        .then(() => true)
        .catch(() => false);
      if (!exists) {
        return [];
  ...
  static async cleanupOldEntries(daysToKeep: number = 90): Promise<number> {
    try {
      ...
      if (removedCount > 0) {
        ...
        console.log(
          `Limpeza do audit log: ${removedCount} entradas antigas removidas`,
        );
      }
