...
import { ValidationError, FileSystemError, CorruptionError } from './backupErrorHandler';
...
export class BackupValidator {
   ...
   */
  static async validateBackup(filepath: string, expectedChecksum?: string): Promise<ValidationResult> {
    ...
    try {
      ...
      if (!fileExists.isValid) {
        ...
        return this.createValidationResult(false, '', 0, errors, warnings, startTime);
      }
      ...
      if (expectedChecksum) {
        const checksumValidation = this.validateChecksum(checksum, expectedChecksum);
        if (!checksumValidation.matches) {
          errors.push(`Checksum não confere. Esperado: ${checksumValidation.expected}, Atual: ${checksumValidation.actual}`);
        }
      }
  ...
  static async calculateChecksum(filepath: string): Promise<string> {
    return new Promise((resolve, reject) => {
      try {
        ...
        stream.on('error', (error: Error) => reject(
          new ValidationError(`Erro ao calcular checksum: ${error.message}`, { filepath, error })
        ));
      } catch (error) {
   ...
   */
  static validateChecksum(actual: string, expected: string): ChecksumValidation {
    return {
   ...
   */
  static async validateSqlStructure(filepath: string): Promise<SqlStructureValidation> {
    try {
      let content: string;
      ...
      const hasCreateStatements = this.SQL_VALIDATION_PATTERNS.CREATE_TABLE.test(content);
      const hasInsertStatements = this.SQL_VALIDATION_PATTERNS.INSERT_INTO.test(content);
      const hasCopyStatements = this.SQL_VALIDATION_PATTERNS.COPY_FROM.test(content);
      const hasValidHeader = this.SQL_VALIDATION_PATTERNS.POSTGRES_DUMP_HEADER.test(content);
      const hasValidFooter = this.SQL_VALIDATION_PATTERNS.DUMP_COMPLETE.test(content);

      // Contar tabelas e registros estimados
      const createMatches = content.match(this.SQL_VALIDATION_PATTERNS.CREATE_TABLE) || [];
      const insertMatches = content.match(this.SQL_VALIDATION_PATTERNS.INSERT_INTO) || [];
      const copyMatches = content.match(this.SQL_VALIDATION_PATTERNS.COPY_FROM) || [];
   ...
   */
  private static async validateFileExists(filepath: string): Promise<ValidationResult> {
    const errors: string[] = [];
    ...
    try {
      ...
      if (!hasValidExtension) {
        errors.push(`Extensão de arquivo não suportada: ${ext}. Suportadas: ${this.SUPPORTED_EXTENSIONS.join(', ')}`);
      }

      return this.createValidationResult(errors.length === 0, '', 0, errors, [], Date.now());
      ...
      if (error && typeof error === 'object' && 'code' in error) {
        switch (error.code) {
          ...
          case 'EACCES':
            ...
            break;
          default:
            errors.push(`Erro ao acessar arquivo: ${error.message || 'Erro desconhecido'}`);
        }
      } else {
   ...
   */
  private static async validateFileSize(filepath: string): Promise<ValidationResult> {
    const errors: string[] = [];
    const warnings: string[] = [];
    ...
    try {
      ...
      if (stats.size === 0) {
        ...
        errors.push(`Arquivo muito pequeno (${stats.size} bytes). Mínimo esperado: ${this.MIN_BACKUP_SIZE} bytes`);
      }
      ...
      return this.createValidationResult(errors.length === 0, '', stats.size, errors, warnings, Date.now());

    } catch (error) {
      errors.push(`Erro ao verificar tamanho do arquivo: ${error instanceof Error ? error.message : 'Erro desconhecido'}`);
      return this.createValidationResult(false, '', 0, errors, warnings, Date.now());
    }
  }
   ...
   */
  private static async validateFileStructure(filepath: string): Promise<ValidationResult> {
    const errors: string[] = [];
    const warnings: string[] = [];
    ...
    try {
      if (filepath.endsWith('.sql') || filepath.endsWith('.sql.gz')) {
        ...
        if (!sqlValidation.hasCreateStatements && !sqlValidation.hasInsertStatements && !sqlValidation.hasCopyStatements) {
        ...
        if (sqlValidation.estimatedRecords > 0) {
          warnings.push(`Estimativa de ${sqlValidation.estimatedRecords} operação(ões) de dados`);
        }
      }

      return this.createValidationResult(errors.length === 0, '', 0, errors, warnings, Date.now());

    } catch (error) {
      errors.push(`Erro ao validar estrutura: ${error instanceof Error ? error.message : 'Erro desconhecido'}`);
      return this.createValidationResult(false, '', 0, errors, warnings, Date.now());
    }
  }
   ...
   */
  private static async checkFileCorruption(filepath: string): Promise<ValidationResult> {
    const errors: string[] = [];
    ...
    try {
      ...
      return this.createValidationResult(!isCorrupted, '', 0, errors, [], Date.now());

    } catch (error) {
      errors.push(`Erro ao verificar corrupção: ${error instanceof Error ? error.message : 'Erro desconhecido'}`);
      return this.createValidationResult(false, '', 0, errors, [], Date.now());
    }
  }
   ...
   */
  private static async readCompressedFile(filepath: string, maxBytes?: number): Promise<string> {
    return new Promise((resolve, reject) => {
      ...
      gunzip.on('error', (error: Error) => reject(
        new ValidationError(`Erro ao descomprimir arquivo: ${error.message}`, { filepath, error })
      ));
  ...
  static generateValidationReport(results: Record<string, ValidationResult>): string {
    ...
    lines.push(`Taxa de sucesso: ${((validFiles / totalFiles) * 100).toFixed(1)}%`);
