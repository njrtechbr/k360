...
export async function exemploValidacaoComChecksum() {
  ...
  try {
    const resultado = await BackupValidator.validateBackup(backupPath, checksumEsperado);
...
export async function exemploValidacaoEstruturaSql() {
  ...
  try {
    ...
    console.log(`  - Cabeçalho PostgreSQL: ${estrutura.hasValidHeader ? '✅' : '❌'}`);
    console.log(`  - Comandos CREATE: ${estrutura.hasCreateStatements ? '✅' : '❌'}`);
    console.log(`  - Comandos INSERT: ${estrutura.hasInsertStatements ? '✅' : '❌'}`);
    console.log(`  - Comandos COPY: ${estrutura.hasCopyStatements ? '✅' : '❌'}`);
    console.log(`  - Footer de conclusão: ${estrutura.hasValidFooter ? '✅' : '❌'}`);
    console.log(`  - Número de tabelas: ${estrutura.tableCount}`);
    console.log(`  - Operações de dados estimadas: ${estrutura.estimatedRecords}`);
...
export async function exemploDeteccaoCorrupcao() {
  ...
  for (const backupPath of backupsParaTestar) {
    try {
      ...
      const estaCorrempido = await BackupValidator.detectFileCorruption(backupPath);
...
export async function exemploValidacaoLote() {
  ...
  try {
    ...
    const resultados = await BackupValidator.validateMultipleBackups(backups, checksumEsperados);
    ...
    for (const [caminho, resultado] of Object.entries(resultados)) {
      ...
      if (resultado.isValid) {
        console.log(`✅ ${nome} - Válido (${(resultado.size / 1024 / 1024).toFixed(2)} MB)`);
        validos++;
      } else {
        console.log(`❌ ${nome} - Inválido (${resultado.errors.length} erro(s))`);
        invalidos++;
      }
    }
...
export async function exemploWorkflowCompleto() {
  ...
  try {
    ...
    if (!estrutura.hasCreateStatements && !estrutura.hasInsertStatements) {
      ...
      console.log(`✅ Backup contém ${estrutura.tableCount} tabela(s) e ${estrutura.estimatedRecords} operação(ões)`);
    }
    ...
    console.log(`  📏 Tamanho: ${(validacao.size / 1024 / 1024).toFixed(2)} MB`);
...
export async function executarTodosExemplos() {
  ...
  console.log('ℹ️ Nota: Os exemplos usam caminhos fictícios. Em produção, use caminhos reais.\n');
