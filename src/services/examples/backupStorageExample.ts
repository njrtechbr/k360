...
async function exemploBackupStorage() {
  ...
  try {
    ...
    allBackups.forEach(backup => {
      console.log(`   - ${backup.filename} (${backup.status}) - ${(backup.size / 1024 / 1024).toFixed(2)}MB`);
    });
    console.log();
    ...
    const successfulBackups = await BackupStorage.listBackups({ status: 'success' });
    ...
    if (stats.oldestBackup) {
      console.log(`   - Backup mais antigo: ${stats.oldestBackup.toLocaleString()}`);
    }
    if (stats.newestBackup) {
      console.log(`   - Backup mais recente: ${stats.newestBackup.toLocaleString()}`);
    }
    console.log();
    ...
    console.log(`🔍 Resultados da busca por "backup": ${searchResults.length} encontrados`);
    console.log();
    ...
    console.log(`   - Espaço liberado: ${(cleanupResult.freedSpace / 1024 / 1024).toFixed(2)}MB`);
