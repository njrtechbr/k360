...
export class ConfigService {
   ...
   */
  static validateGamificationConfig(
    ...
    if (config.pointsPerEvaluation) {
      ...
      if (points.rating1 >= 0) {
        warnings.push(
          "Considere usar pontos negativos para rating 1 para desencorajar baixas avaliações",
        );
      }

      if (points.rating5 < 10) {
        warnings.push(
          "Pontos para rating 5 podem ser muito baixos para motivar",
        );
      }
    }
    ...
    if (config.multipliers) {
      ...
      Object.entries(mult).forEach(([key, value]) => {
        if (typeof value === "number") {
          ...
          if (value > 10) {
            warnings.push(
              `Multiplicador ${key} pode ser muito alto (${value})`,
            );
          }
        }
      });
    ...
    if (config.sessions) {
      ...
      if (
        sessions.minEvaluationsForBonus &&
        sessions.minEvaluationsForBonus < 1
      ) {
        errors.push("Mínimo de avaliações para bônus deve ser pelo menos 1");
      }

      if (
        sessions.sessionTimeoutMinutes &&
        sessions.sessionTimeoutMinutes < 5
      ) {
        warnings.push(
          "Timeout de sessão muito baixo pode causar problemas de usabilidade",
        );
      }
    }
   ...
   */
  static validateAchievementConfig(
      ...
      Object.entries(config.categories).forEach(
        ([categoryName, categoryConfig]) => {
          if (categoryConfig.weight < 0) {
            errors.push(
              `Peso da categoria ${categoryName} não pode ser negativo`,
            );
          }

          if (categoryConfig.weight > 5) {
            warnings.push(
              `Peso da categoria ${categoryName} pode ser muito alto`,
            );
          }
        },
      );
    ...
    if (config.difficulty) {
      ...
      for (let i = 1; i < difficulties.length; i++) {
        ...
        if (currDiff.xpReward <= prevDiff.xpReward) {
          warnings.push(
            `Recompensa XP para ${currName} deveria ser maior que ${prevName}`,
          );
        }
      }
    }
   ...
   */
  static validateLevelTrackConfig(
    config: Partial<LevelTrackConfig>,
  ): ConfigValidationResult {
    const errors: string[] = [];
    const warnings: string[] = [];
    ...
    if (config.xpFormula) {
      ...
      if (formula.multiplier && formula.multiplier > 3) {
        warnings.push(
          "Multiplicador muito alto pode tornar progressão muito rápida",
        );
      }
    }
    ...
    if (config.milestones) {
      ...
      if (milestones.length === 0) {
        warnings.push(
          "Considere adicionar marcos para melhor experiência do usuário",
        );
      }
  ...
  static validateAllConfigs(
    ...
    if (settings.gamificationConfig) {
      const result = this.validateGamificationConfig(
        settings.gamificationConfig,
      );
    ...
    if (metrics.participationRate < 0.5) {
      ...
      optimized.gamificationConfig = {
        ...
        pointsPerEvaluation: {
          ...
          rating4: Math.round(
            currentConfig.gamificationConfig.pointsPerEvaluation.rating4 * 1.2,
          ),
          rating5: Math.round(
            currentConfig.gamificationConfig.pointsPerEvaluation.rating5 * 1.2,
          ),
        },
      };
    }
    ...
    if (metrics.retentionRate < 0.7) {
      ...
      optimized.gamificationConfig = {
        ...
        multipliers: {
          ...
          consistencyBonus: Math.min(
            2.0,
            currentConfig.gamificationConfig.multipliers.consistencyBonus * 1.1,
          ),
          perfectWeek: Math.min(
            3.0,
            currentConfig.gamificationConfig.multipliers.perfectWeek * 1.1,
          ),
        },
      };
    }
      ...
      optimized.achievementConfig = {
        ...
        difficulty: {
          ...
          easy: {
            ...
            xpReward: Math.round(
              currentConfig.achievementConfig.difficulty.easy.xpReward * 1.3,
            ),
          },
        },
      };
  ...
  static importConfig(configJson: string): {
    ...
    try {
      ...
      if (
        !parsed.gamificationConfig ||
        !parsed.achievementConfig ||
        !parsed.levelTrackConfig
      ) {
        return {
          success: false,
