...
describe('BackupService - Unit Tests', () => {
  beforeEach(() => {
    ...
    (BackupValidator.calculateChecksum as jest.Mock).mockResolvedValue('test-checksum-abc123');
    (BackupValidator.validateMultipleBackups as jest.Mock).mockResolvedValue({});
    (BackupValidator.generateValidationReport as jest.Mock).mockReturnValue('Relatório de teste');
    (BackupValidator.detectFileCorruption as jest.Mock).mockResolvedValue(false);
    ...
    (fs.readFile as jest.Mock).mockResolvedValue(JSON.stringify(defaultRegistry));
  ...
  describe('Métodos Utilitários', () => {
    it('deve gerar nome de arquivo com timestamp correto', () => {
      ...
      expect(filename).toMatch(/^backup_\d{4}-\d{2}-\d{2}_\d{2}-\d{2}-\d{2}\.sql$/);
    });
  ...
  describe('listBackups', () => {
    ...
    it('deve retornar backups ordenados por data', async () => {
      const mockRegistry = {
        ...
        lastCleanup: null,
        settings: { maxBackups: 50, retentionDays: 30, defaultDirectory: './backups' }
      };

      (fs.readFile as jest.Mock).mockResolvedValue(JSON.stringify(mockRegistry));
  ...
  describe('validateBackup', () => {
    it('deve retornar true para arquivo SQL válido', async () => {
      ...
      expect(BackupValidator.validateBackup).toHaveBeenCalledWith('test.sql', undefined);
    });
    ...
    it('deve retornar true para arquivo com checksum válido', async () => {
      ...
      const isValid = await BackupService.validateBackup('test.sql', 'expected-checksum');
      
      expect(isValid).toBe(true);
      expect(BackupValidator.validateBackup).toHaveBeenCalledWith('test.sql', 'expected-checksum');
    });
    ...
    it('deve retornar false quando BackupValidator lança exceção', async () => {
      (BackupValidator.validateBackup as jest.Mock).mockRejectedValue(new Error('Erro de validação'));
  ...
  describe('getBackupValidationDetails', () => {
    it('deve retornar detalhes completos da validação', async () => {
      ...
      (BackupValidator.validateBackup as jest.Mock).mockResolvedValue(mockValidationResult);

      const details = await BackupService.getBackupValidationDetails('test.sql', 'abc123');
      
      expect(details).toEqual(mockValidationResult);
      expect(BackupValidator.validateBackup).toHaveBeenCalledWith('test.sql', 'abc123');
    });
  });
  ...
  describe('Validação Múltipla de Backups', () => {
    it('deve validar múltiplos backups com sucesso', async () => {
      ...
      const mockValidationResults = {
        '/path/backup1.sql': { isValid: true, checksum: 'checksum1', errors: [] },
        '/path/backup2.sql': { isValid: true, checksum: 'checksum2', errors: [] }
      };

      (BackupValidator.validateMultipleBackups as jest.Mock).mockResolvedValue(mockValidationResults);
      (BackupValidator.generateValidationReport as jest.Mock).mockReturnValue('Relatório de validação');
    ...
    it('deve lançar erro quando nenhum backup é encontrado', async () => {
      ...
      await expect(BackupService.validateMultipleBackups(['nonexistent']))
  ...
  describe('runIntegrityCheck', () => {
    it('deve executar verificação de integridade com sucesso', async () => {
      const mockRegistry = {
        backups: [
          { id: '1', filepath: '/path/backup1.sql', checksum: 'checksum1', filename: 'backup1.sql' },
          { id: '2', filepath: '/path/backup2.sql', checksum: 'checksum2', filename: 'backup2.sql' }
        ],
        lastCleanup: new Date(),
        settings: { maxBackups: 50, retentionDays: 30, defaultDirectory: './backups' }
      };
      ...
      (BackupValidator.validateMultipleBackups as jest.Mock).mockResolvedValue(mockValidationResults);
      (BackupValidator.generateValidationReport as jest.Mock).mockReturnValue('Relatório completo');
    ...
    it('deve detectar backups inválidos', async () => {
      ...
      const mockValidationResults = {
        '/path/backup1.sql': { isValid: true, checksum: 'checksum1', errors: [] },
        '/path/backup2.sql': { isValid: false, checksum: '', errors: ['Arquivo corrompido'] }
      };

      (BackupValidator.validateMultipleBackups as jest.Mock).mockResolvedValue(mockValidationResults);
      (BackupValidator.generateValidationReport as jest.Mock).mockReturnValue('Relatório com erros');
  ...
  describe('detectCorruptedBackups', () => {
    it('deve detectar backups corrompidos', async () => {
      const mockRegistry = {
        backups: [
          { id: '1', filepath: '/path/backup1.sql', filename: 'backup1.sql', createdAt: new Date(), size: 1000 },
          { id: '2', filepath: '/path/backup2.sql', filename: 'backup2.sql', createdAt: new Date(), size: 2000 }
        ],
        lastCleanup: new Date(),
        settings: { maxBackups: 50, retentionDays: 30, defaultDirectory: './backups' }
      };
    ...
    it('deve tratar erros de acesso a arquivo como corrupção', async () => {
      ...
      (fs.readFile as jest.Mock).mockResolvedValue(JSON.stringify(mockRegistry));
      (BackupValidator.detectFileCorruption as jest.Mock).mockRejectedValue(new Error('Arquivo não encontrado'));
  ...
  describe('Operações de Arquivo e Registry', () => {
    ...
    it('deve carregar registry existente', async () => {
      const mockRegistry = {
        ...
        lastCleanup: new Date(),
        settings: { maxBackups: 100, retentionDays: 60, defaultDirectory: './custom' }
      };
  ...
  describe('Mocks do Sistema de Arquivos', () => {
    it('deve simular verificação de diretório existente', async () => {
      ...
      await expect((BackupService as any).ensureDirectoryExists('./test-dir'))
    ...
    it('deve simular criação de diretório quando não existe', async () => {
      ...
      await expect((BackupService as any).ensureDirectoryExists('./new-dir'))
        .resolves.not.toThrow();
    ...
    it('deve simular verificação de espaço em disco', async () => {
      ...
      await expect((BackupService as any).checkDiskSpace('./test-dir'))
        .resolves.not.toThrow();
    });
  ...
  describe('Mocks do BackupValidator', () => {
    ...
    it('deve simular cálculo de checksum', async () => {
      (BackupValidator.calculateChecksum as jest.Mock).mockResolvedValue('calculated-checksum');
    ...
    it('deve simular detecção de corrupção', async () => {
      (BackupValidator.detectFileCorruption as jest.Mock).mockResolvedValue(true);

      const isCorrupted = await BackupValidator.detectFileCorruption('corrupted.sql');
    ...
    it('deve simular validação múltipla', async () => {
      ...
      (BackupValidator.validateMultipleBackups as jest.Mock).mockResolvedValue(mockResults);

      const results = await BackupValidator.validateMultipleBackups(['file1.sql', 'file2.sql'], {});
