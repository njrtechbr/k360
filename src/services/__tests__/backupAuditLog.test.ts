...
describe('BackupAuditLogger', () => {
  ...
  describe('logOperation', () => {
    ...
    it('deve continuar mesmo se log falhar', async () => {
      ...
      await expect(BackupAuditLogger.logOperation(entry)).resolves.toBeUndefined();
    });
  });
  ...
  describe('getAuditEntries', () => {
    ...
    it('deve aplicar filtros corretamente', async () => {
      ...
      const entries = await BackupAuditLogger.getAuditEntries({ success: true });
    ...
    it('deve ignorar linhas inválidas', async () => {
      ...
      expect(entries).toHaveLength(2);
      expect(console.warn).toHaveBeenCalledWith('Linha inválida no audit log:', 'linha inválida');
    });
  });
  ...
  describe('createAuditEntry', () => {
    it('deve criar entrada de audit corretamente', () => {
      const user = { id: 'user-1', email: 'test@test.com', role: 'ADMIN' as Role };
    ...
    it('deve criar entrada mínima sem opções', () => {
      const user = { id: 'user-1', email: 'test@test.com', role: 'SUPERVISOR' as Role };
