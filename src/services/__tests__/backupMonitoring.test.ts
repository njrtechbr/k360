...
jest.mock('@/config/backupMonitoring', () => ({
  loadMonitoringConfig: () => ({
    cleanup: { enabled: true, schedule: '0 2 * * *', retentionDays: 30, maxBackups: 50 },
    healthCheck: { enabled: true, schedule: '*/30 * * * *', diskSpaceThreshold: 90, alertOnFailure: true },
    alerts: { maxAlerts: 100, retentionDays: 7, emailNotifications: false },
    metrics: { collectInterval: 5, retentionDays: 30, enableDetailedMetrics: true }
  })
}));
...
describe('BackupMonitoring', () => {
  ...
  describe('performAutomaticCleanup', () => {
    it('deve executar limpeza automática corretamente', async () => {
      ...
      expect(mockAuditLog.log).toHaveBeenCalledWith('system', 'cleanup_started', {
        retentionDays: 7,
        maxBackups: 10
      });
      expect(mockAuditLog.log).toHaveBeenCalledWith('system', 'cleanup_completed', {
        totalDeleted: 5,
        deletedByAge: 3,
        deletedByCount: 2
      });
    ...
    it('deve lidar com erros na limpeza', async () => {
      ...
      mockStorage.getConfig.mockRejectedValue(new Error('Erro de configuração'));

      await monitoring.performAutomaticCleanup();

      expect(mockAuditLog.log).toHaveBeenCalledWith('system', 'cleanup_failed', {
  ...
  describe('performHealthCheck', () => {
    it('deve retornar status saudável quando tudo está OK', async () => {
      ...
      jest.spyOn(monitoring as any, 'checkBackupIntegrity').mockResolvedValue(true);
      jest.spyOn(monitoring as any, 'checkDatabaseConnection').mockResolvedValue(true);
    ...
    it('deve retornar status crítico quando há problemas graves', async () => {
      jest.spyOn(monitoring as any, 'checkDiskSpace').mockResolvedValue(true);
      jest.spyOn(monitoring as any, 'checkBackupIntegrity').mockResolvedValue(false);
      jest.spyOn(monitoring as any, 'checkDatabaseConnection').mockResolvedValue(false);
      jest.spyOn(monitoring as any, 'checkPermissions').mockResolvedValue(true);
    ...
    it('deve retornar status de atenção para problemas menores', async () => {
      ...
      jest.spyOn(monitoring as any, 'checkBackupIntegrity').mockResolvedValue(true);
      jest.spyOn(monitoring as any, 'checkDatabaseConnection').mockResolvedValue(true);
      jest.spyOn(monitoring as any, 'checkPermissions').mockResolvedValue(false);
  ...
  describe('createAlert', () => {
    it('deve criar alerta corretamente', async () => {
      await monitoring.createAlert('error', 'Teste de erro', { detail: 'teste' });
  ...
  describe('resolveAlert', () => {
    it('deve resolver alerta existente', async () => {
      ...
      expect(mockAuditLog.log).toHaveBeenCalledWith('system', 'alert_resolved', { alertId });
