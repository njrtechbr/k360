...
export const ISODateSchema = z.string().datetime('Data deve estar no formato ISO');
...
export const XPSchema = z.number()
...
export const AttendantSchema = z.object({
  ...
  portaria: z.string().max(100, 'Portaria deve ter no máximo 100 caracteres').nullable(),
  situacao: z.string().max(100, 'Situação deve ter no máximo 100 caracteres').nullable(),
...
export const UpdateAttendantSchema = AttendantSchema.partial().omit({ id: true });
...
export const UpdateEvaluationSchema = EvaluationSchema.partial().omit({ id: true, xpGained: true });
...
export const PaginationSchema = z.object({
  ...
  limit: z.number().int().min(1, 'Limite deve ser pelo menos 1').max(100, 'Limite deve ser no máximo 100').default(10),
