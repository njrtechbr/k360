...
const SuggestAttendantInputSchema = z.object({
  agentNames: z
    .array(z.string())
    .describe("Uma lista de nomes de agentes extraídos de um arquivo CSV."),
  attendants: z
    .array(
      z.object({
        id: z.string(),
    ...
    .describe(
      "A lista de atendentes cadastrados no sistema com seus IDs e nomes.",
    ),
});
    ...
    .array(
      z.object({
        agentName: z
          .string()
          .describe(
            "O nome original do agente, conforme fornecido na entrada.",
          ),
        attendantId: z
          .string()
          .nullable()
          .describe(
            "O ID do atendente correspondente sugerido ou null se nenhuma correspondência clara for encontrada.",
          ),
      }),
    )
...
export async function suggestAttendants(
  input: SuggestAttendantInput,
): Promise<SuggestAttendantOutput> {
...
const suggestAttendantFlow = ai.defineFlow(
  ...
  async (input) => {
    ...
    if (!output) {
      throw new Error(
        "A IA não retornou uma resposta válida para a sugestão de atendentes.",
      );
    }
    return output;
  },
